...
open Causality_effects.Effects
...
let test_effect_creation () =
  ...
  (match run_with_effects test_allocation with
  | Ok () -> ()
  | Error exn ->
      Printf.printf "✗ Failed to allocate resource: %s\n"
        (Printexc.to_string exn));
  ...
  | Ok () -> ()
  | Error exn ->
      Printf.printf "✗ Failed to check constraint: %s\n"
        (Printexc.to_string exn));
  ...
  | Ok () -> ()
  | Error exn ->
      Printf.printf "✗ Failed to generate witness: %s\n"
        (Printexc.to_string exn)
  ...
  | Ok resources ->
      ...
      List.iteri
        (fun i resource ->
          ...
          | Error exn ->
              Printf.printf "  - Resource %d consumption failed: %s\n" i
                (Printexc.to_string exn))
        resources
  | Error exn ->
