...
use crate::resource::epoch::EpochId;
use crate::types::{Hash256, Domain};
...
impl RegisterArchive {
    /// Create a new register archive from a register
    pub fn from_register(
        ...
        // Serialize register contents
        let contents = register.contents.to_bytes();
        ...
        let content_hash = Hash256::digest(&contents);
        ...
        let mut archive = Self {
            ...
            block_height,
            epoch,
            original_state: register.state,
            contents,
            metadata: register.metadata.clone(),
            compression,
            content_hash,
            archive_hash: Hash256::default(), // Set later
        };
        ...
        hash_data.extend_from_slice(&archive.version.to_be_bytes());
        hash_data.extend_from_slice(archive.register_id.to_string().as_bytes());
        hash_data.extend_from_slice(archive.domain.to_string().as_bytes());
        hash_data.extend_from_slice(&archive.created_at.to_be_bytes());
        hash_data.extend_from_slice(&archive.archived_at.to_be_bytes());
        hash_data.extend_from_slice(&archive.block_height.to_be_bytes());
        hash_data.extend_from_slice(&archive.epoch.to_be_bytes());
        hash_data.extend_from_slice(&[archive.original_state as u8]);
        hash_data.extend_from_slice(&archive.contents);
        hash_data.extend_from_slice(archive.content_hash.as_bytes());
        
        for (key, value) in &archive.metadata {
            hash_data.extend_from_slice(key.as_bytes());
            hash_data.extend_from_slice(value.as_bytes());
        }
        
        archive.archive_hash = Hash256::digest(&hash_data);
...
impl ArchiveManager {
    ...
    pub fn archive_register(
        ...
        // Create archive
        let mut archive = RegisterArchive::from_register(
            register,
            epoch,
            block_height,
...
mod tests {
    ...
    fn test_archive_creation() -> Result<()> {
        ...
        // Create archive
        let archive = RegisterArchive::from_register(
            &register,
            1, // epoch
            100, // block height
    ...
    fn test_compression() -> Result<()> {
        ...
        let uncompressed = RegisterArchive::from_register(
            &register,
            1,
            100,
            CompressionFormat::None,
        ...
        let gzip = RegisterArchive::from_register(
            &register,
            1,
            100,
            CompressionFormat::Gzip,
        ...
        let zstd = RegisterArchive::from_register(
            &register,
            1,
            100,
            CompressionFormat::Zstd,
