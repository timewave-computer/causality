// Continuation module for Causality Effect System
//
// This module provides functionality for representing and manipulating
// continuations, which are used to handle the results of effects.
...
pub fn constant<I, O>(value: O) -> Box<dyn Continuation<I, O>>
where
    I: Debug + Send + 'static,
...
/// A continuation that returns the input unchanged
///
/// This is a convenience function for creating an identity continuation.
pub fn identity<T>() -> Box<dyn Continuation<T, T>>
where
