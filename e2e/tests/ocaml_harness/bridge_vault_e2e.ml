...
let step_1_compile_dsl () =
  ...
  let ocaml_compile_cmd = "cd /Users/hxrts/projects/timewave/reverse-causality/e2e/tests/ocaml_harness && dune build bridge_vault_scenario.exe" in
  try
    let (ocaml_output, ocaml_exit_code) = run_command_with_timeout ocaml_compile_cmd 15 in
    ...
    | WEXITED 0 -> 
      ...
      let lisp_compile_cmd = "cd /Users/hxrts/projects/timewave/reverse-causality && ./target/debug/causality compile --input e2e/tests/ocaml_harness/bridge_vault_scenario.lisp --output /tmp/bridge_vault.ir --format intermediate --verbose" in
      let (lisp_output, lisp_exit_code) = run_command_with_timeout lisp_compile_cmd 30 in
      ...
      | WEXITED 0 ->
        ...
        if Sys.file_exists "/tmp/bridge_vault.ir" then (
          ...
          Failure "IR file was not created"
        )
...
let step_2_run_simulation () =
  ...
  let simulation_cmd = "cd /Users/hxrts/projects/timewave/reverse-causality && ./target/debug/causality simulate --input /tmp/bridge_vault.ir --cost-analysis --chains ethereum,polygon --gas-price-gwei 20 --verbose" in
  
  try
    let (simulation_output, simulation_exit_code) = run_command_with_timeout simulation_cmd 30 in
    ...
    | WEXITED 0 ->
      ...
      if metrics.total_gas_cost > 0 && metrics.total_gas_cost < 1000000 then (
        if metrics.success_probability > 0.9 then (
          ...
          Failure (Printf.sprintf "Success probability too low: %.3f" metrics.success_probability)
        )
      ) else (
        Failure (Printf.sprintf "Gas cost out of range: %d" metrics.total_gas_cost)
      )
...
let step_3_compile_zk_circuit () =
  ...
  let zk_compile_cmd = "cd /Users/hxrts/projects/timewave/reverse-causality && ./target/debug/causality zk-compile --input /tmp/bridge_vault.ir --output /tmp/bridge_vault_circuit.zk --privacy-level high --proof-system groth16 --verbose" in
  ...
  try
    ...
    | WEXITED 0 ->
      ...
      if Sys.file_exists "/tmp/bridge_vault_circuit.zk" then (
        printf "✅ ZK circuit file created: /tmp/bridge_vault_circuit.zk\n";
        Success "ZK circuit compiled with Groth16 proof system"
      ) else (
        Failure "ZK circuit file was not created"
      )
...
let step_4_verify_zk_proof () =
  ...
  (* Create witness data *)
  let witness_cmd = "echo '{\"private_inputs\": [1000000000, 995000000], \"public_inputs\": [8]}' > /tmp/bridge_vault_witness.json" in
  ...
  if witness_exit_code = WEXITED 0 then (
    ...
    let zk_verify_cmd = "cd /Users/hxrts/projects/timewave/reverse-causality && ./target/debug/causality zk-verify --circuit /tmp/bridge_vault_circuit.zk --witness-file /tmp/bridge_vault_witness.json --mock-runtime --verbose" in
    
    try
      let (verify_output, verify_exit_code) = run_command_with_timeout zk_verify_cmd 30 in
    ...
    Failure "Failed to create witness data"
  )
...
let step_5_submit_transactions () =
  ...
  let submit_cmd = "cd /Users/hxrts/projects/timewave/reverse-causality && ./target/debug/causality submit-transaction --circuit /tmp/bridge_vault_circuit.zk --proof /tmp/bridge_vault_circuit.zk --target-chains ethereum,polygon --dry-run --verbose" in
  
  try
    let (submit_output, submit_exit_code) = run_command_with_timeout submit_cmd 30 in
...
let step_6_generate_report () =
  ...
  let report_cmd = "cd /Users/hxrts/projects/timewave/reverse-causality && ./target/debug/causality generate-report --scenario bridge-vault-deposit --include-proofs --include-gas-analysis --include-privacy-analysis --output /tmp/bridge_vault_report.json --verbose" in
  
  try
    let (report_output, report_exit_code) = run_command_with_timeout report_cmd 30 in
    ...
    | WEXITED 0 ->
      ...
      if Sys.file_exists "/tmp/bridge_vault_report.json" then (
        ...
        let (snippet_output, _) = run_command_with_timeout report_snippet_cmd 5 in
...
let run_comprehensive_test () =
  ...
  if success_count = total_count then (
    printf "\n🎉 ALL TESTS PASSED! Cross-chain bridge and vault deposit E2E test completed successfully.\n";
