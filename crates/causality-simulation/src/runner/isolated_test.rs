// Purpose: Isolated tests for the InMemoryRunner implementation
//
...
use causality_core::effect::{Effect, EffectContext, EffectType, EffectError};
use causality_core::effect::outcome::{EffectOutcome, EffectResult};
...
impl TestAgentFactory {
    fn new() -> Self {
        Self {
            was_called: Arc::new(std::sync::atomic::AtomicBool::new(false)),
        }
    }

    fn was_called(&self) -> bool {
        self.was_called.load(std::sync::atomic::Ordering::SeqCst)
    }
}
...
struct MockAgent {
    id: AgentId,
    on_run: Arc<std::sync::atomic::AtomicBool>,
    on_shutdown: Arc<std::sync::atomic::AtomicBool>,
}
...
impl MockAgent {
    fn new(id: AgentId) -> Self {
        Self {
            id,
            on_run: Arc::new(std::sync::atomic::AtomicBool::new(false)),
            on_shutdown: Arc::new(std::sync::atomic::AtomicBool::new(false)),
        }
    }

    fn was_run(&self) -> bool {
        self.on_run.load(std::sync::atomic::Ordering::SeqCst)
    }

    fn was_shutdown(&self) -> bool {
        self.on_shutdown.load(std::sync::atomic::Ordering::SeqCst)
    }
}
...
impl SimulatedAgent for MockAgent {
    async fn run(&self, _config: SimulationAgentConfig) -> Result<()> {
        self.on_run.store(true, std::sync::atomic::Ordering::SeqCst);
    ...
    async fn shutdown(&self) -> Result<()> {
        self.on_shutdown.store(true, std::sync::atomic::Ordering::SeqCst);
        Ok(())
    }
}
...
mod tests {
    ...
    async fn test_in_memory_runner_lifecycle() -> Result<()> {
        ...
        timeout(Duration::from_secs(10), async {
            let (runner, agent_factory, effect_factory, _observer_registry, _log_storage) = create_test_runner();
