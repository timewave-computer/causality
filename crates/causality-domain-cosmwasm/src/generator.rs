...
use causality_ir::effect_node::EffectNode;
use causality_ir::resource_node::ResourceNode;
use causality_ir::TEGFragment;
use causality_codegen_core::{CodeGenConfig, GeneratedCode, CodeGenerator};
use causality_codegen_core::target::{Target, BaseTarget, TargetCapabilities};
use case::CaseExt;
...
"#.to_string()
    ...
    fn generate_resource_structs(&self, teg: &TemporalEffectGraph) -> String {
        ...
        for (type_name, resource) in resource_types {
            ...
            if let serde_json::Value::Object(obj) = serde_json::json!(resource.metadata) {
...
impl Target for CosmWasmGenerator {
    ...
    fn generate_code(&self, teg: &TemporalEffectGraph, _config: &CodeGenConfig) -> Result<GeneratedCode> {
        ...
        metadata.insert("generator".to_string(), "cosmwasm".to_string());
        metadata.insert("effect_count".to_string(), format!("{}", teg.effects().len()));
        metadata.insert("resource_count".to_string(), format!("{}", teg.resources().len()));
        metadata.insert("domains".to_string(), format!("{}", teg.domains.len()));
        
        // Create src directory
        let mut src_dir = SourceDirectory::new("src");
        
        // Add Cargo.toml
        files.insert("Cargo.toml".to_string(), SourceFile::new(self.generate_cargo_toml()));
        
        // Add lib.rs
        let lib_content = format!(
            "//! CosmWasm contract generated from TEG\n\nmod contract;\nmod msg;\nmod state;\n"
        );
        src_dir.add_file("lib.rs".to_string(), SourceFile::new(lib_content));
        
        // Add contract.rs
        let contract_content = self.generate_contract_structure(teg) + 
                               &self.generate_entry_points() +
                               &self.generate_execute_handlers(teg) +
                               &self.generate_query_handlers(teg);
        src_dir.add_file("contract.rs".to_string(), SourceFile::new(contract_content));
        
        // Add msg.rs
        let msg_content = self.generate_message_types(teg);
        src_dir.add_file("msg.rs".to_string(), SourceFile::new(msg_content));
        
        // Add state.rs
        let state_content = self.generate_resource_structs(teg);
        src_dir.add_file("state.rs".to_string(), SourceFile::new(state_content));
        
        // Add tests directory
        let mut tests_dir = SourceDirectory::new("tests");
        tests_dir.add_file("integration.rs".to_string(), SourceFile::new(self.generate_test_file()));
        
        // Add schema directory
        let mut schema_dir = SourceDirectory::new("schema");
        schema_dir.add_file("generate.rs".to_string(), SourceFile::new(self.generate_schema_file()));
        
        // Add directories
        directories.insert("src".to_string(), src_dir);
        directories.insert("tests".to_string(), tests_dir);
        directories.insert("schema".to_string(), schema_dir);
