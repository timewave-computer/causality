...
use causality_core::{
    Affine, Linear, LinearResource, LinearityError, Relevant, Unrestricted,
};
...
fn test_relevant_resource_must_be_used() {
    ...
    let resource: LinearResource<String, Relevant> =
        LinearResource::new("hello".to_string());
...
fn test_unrestricted_resource_can_be_copied_and_dropped() {
    ...
    let resource: LinearResource<String, Unrestricted> =
        LinearResource::new("world".to_string());
...
fn test_multiple_use_error() {
    ...
    match resource.try_consume() {
        Err(LinearityError::MultipleUse { resource: _ }) => {
            // Expected error for multiple use
        }
        _ => panic!("Expected multiple use error"),
    }
}

#[test]
fn test_copy_after_drop_error() {
    let mut resource: LinearResource<String, Unrestricted> =
        LinearResource::new("test".to_string());
    ...
    match resource.copy_resource() {
        Err(LinearityError::UseAfterConsumption { resource: _ }) => {
            // Expected error
        }
        _ => panic!("Expected UseAfterConsumption error"),
    }
}
...
fn test_unrestricted_copy_marks_as_used() {
    // Unrestricted resources can be used any number of times
    let resource: LinearResource<String, Unrestricted> =
        LinearResource::new("unrestricted".to_string());
