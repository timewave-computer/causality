...
pub enum TypeCheckError {
    ...
    #[error(
        "Session protocol mismatch: cannot perform {operation} on {session_type:?}"
    )]
...
impl TypeContext {
    ...
    pub fn lookup_channel(
        &self,
        name: &str,
    ) -> Result<&SessionType, TypeCheckError> {
        self.session_env
            .lookup_channel(name)
...
pub fn type_check(
    ...
    match &term.kind {
        ...
        TermKind::Literal(lit) => Ok(literal_type(lit)),

        TermKind::Unit => Ok(TypeInner::Base(BaseType::Unit)),
        ...
        TermKind::LetTensor {
            ...
            match tensor_ty {
                ...
                _ => Err(TypeCheckError::InvalidTensorElimination(Box::new(
                    tensor_ty,
                ))),
            }
        }
        ...
        TermKind::Inl { value, sum_type } => {
            ...
            match sum_type {
                ...
                _ => Err(TypeCheckError::TypeMismatch {
                    expected: Box::new(TypeInner::Sum(
        ...
        TermKind::Send { channel, value } => {
            ...
            match channel_ty {
                ...
                _ => Err(TypeCheckError::InvalidSessionOperation(Box::new(
                    channel_ty,
                ))),
            }
        }
                        ...
                        SessionType::InternalChoice(ref choices) => {
                            ...
                            for (choice_label, continuation) in choices {
                                if choice_label == label {
                                    // Update channel to continuation type
                                    if let TermKind::Var(channel_name) =
                                        &channel.kind
                                    {
                                        ctx.update_channel(
                                            channel_name,
                    ...
                    match *session_ty {
                        SessionType::ExternalChoice(ref choices) => {
                            ...
                            for (label, branch_term) in branches {
                                ...
                                if !found {
                                    return Err(
                                        TypeCheckError::ChoiceLabelNotFound {
                                            label: label.clone(),
                                            session_type: *session_ty,
                                        },
                                    );
        ...
        TermKind::Fork {
            ...
            ctx.bind_variable(
                client_var.clone(),
                TypeInner::Session(Box::new(client_session)),
            )?;
            ctx.bind_variable(
                server_var.clone(),
                TypeInner::Session(Box::new(server_session)),
            )?;
        ...
        TermKind::Wait { channel, body } => {
            ...
            match channel_ty {
                TypeInner::Session(session_ty) => match *session_ty {
                    SessionType::End => type_check(ctx, body),
                    _ => Err(TypeCheckError::SessionProtocolMismatch {
...
pub fn infer_session_types(
    ctx: &mut TypeContext,
    term: &Term,
) -> Result<(TypeInner, Vec<SessionTypeConstraint>), TypeCheckError> {
            ...
            match &channel.kind {
                TermKind::Var(channel_name) => {
                    ...
                    for (label, branch_term) in branches {
                        ...
                        let branch_ty =
                            infer_with_constraints(ctx, branch_term, constraints)?;
                    ...
                    let expected_session =
                        SessionType::ExternalChoice(branch_constraints);
                    constraints.push(SessionTypeConstraint::ChannelType(
                        channel_name.clone(),
                        expected_session,
                    ));
...
pub fn solve_constraints(
    constraints: &[SessionTypeConstraint],
) -> Result<HashMap<String, SessionType>, TypeCheckError> {
            ...
            SessionTypeConstraint::SupportsOperation(session_type, operation) => {
                ...
                match (session_type, operation) {
                    ...
                    (
                        ...
                        SessionOperation::Select(_),
                    ) => {}
                    (
                        SessionType::ExternalChoice(_),
                        SessionOperation::Branch(_),
                    ) => {}
...
mod tests {
    ...
    fn test_basic_type_checking() {
        ...
        // Test literals
        assert_eq!(
            type_check(&mut ctx, &Term::unit()).unwrap(),
            TypeInner::Base(BaseType::Unit)
        );
        assert_eq!(
            type_check(&mut ctx, &Term::literal(Literal::Bool(true))).unwrap(),
            TypeInner::Base(BaseType::Bool)
        );
        assert_eq!(
            type_check(&mut ctx, &Term::literal(Literal::Int(42))).unwrap(),
    ...
    fn test_lambda_types() {
        ...
        let lambda_term =
            Term::lambda_typed("x", TypeInner::Base(BaseType::Int), Term::var("x"));
    ...
    fn test_session_send_receive() {
        ...
        // Bind a channel
        ctx.bind_channel("ch".to_string(), session_type.clone())
            .unwrap();
        ctx.bind_variable(
            "ch".to_string(),
    ...
    fn test_session_type_inference_send() {
        ...
        ctx.bind_channel("ch".to_string(), placeholder_session.clone())
            .unwrap();
        ctx.bind_variable(
            "ch".to_string(),
            TypeInner::Session(Box::new(placeholder_session)),
        )
        .unwrap();
        ...
        if let SessionTypeConstraint::ChannelType(channel_name, session_type) =
            &constraints[0]
        {
    ...
    fn test_constraint_solving() {
        let constraints = vec![SessionTypeConstraint::ChannelType(
            "ch1".to_string(),
            SessionType::Send(
                Box::new(TypeInner::Base(BaseType::Int)),
                Box::new(SessionType::End),
    ...
    fn test_transform_application() {
        ...
        let application =
            Term::apply_transform(transform_term, Term::literal(Literal::Int(42)));
    ...
    fn test_located_computation() {
        ...
        let at_term = Term::at(
            crate::lambda::base::Location::Remote(crate::EntityId::from_content(
    ...
    fn test_transform_with_remote_location() {
        ...
        match transform_type {
            TypeInner::Transform { location, .. } => {
                assert_eq!(
                    location,
                    crate::lambda::base::Location::Remote(
                        crate::EntityId::from_content(
                            &"gpu_server".as_bytes().to_vec()
                        )
                    )
                );
