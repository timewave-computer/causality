...
impl TimeMap {
    ...
    pub fn get_snapshot(&self) -> causality_types::Result<TimeMapSnapshot> {
...
pub trait TimeService {
    ...
    fn get_snapshot(&self) -> causality_types::Result<TimeMapSnapshot>;
    fn validate_snapshot(&self, snapshot: &TimeMapSnapshot) -> causality_types::Result<bool>;
}

// Time observer trait for components that need to respond to time events
pub trait TimeObserver {
    fn on_time_event(&mut self, event: TimeEvent) -> causality_types::Result<()>;
}
