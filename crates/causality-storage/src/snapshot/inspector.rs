// Snapshot inspection utilities
// Original file: src/snapshot/inspector.rs
...
use causality_engine::{ExecutionContext, Value, ContextId};
...
impl StateInspector for ContextStateInspector {
    ...
    fn get_position_info(
        ...
        let event_type = match event {
            causality_engine::ExecutionEvent::FunctionCall { .. } => "FunctionCall",
            causality_engine::ExecutionEvent::FunctionReturn { .. } => "FunctionReturn",
            causality_engine::ExecutionEvent::VariableAssignment { .. } => "VariableAssignment",
            causality_engine::ExecutionEvent::EffectApplied { .. } => "EffectApplied",
            causality_engine::ExecutionEvent::ErrorThrown { .. } => "ErrorThrown",
            _ => "Unknown",
        }.to_string();
        
        // Extract function name and source location if available
        let (function_name, source_location) = match event {
            causality_engine::ExecutionEvent::FunctionCall { function_name, source_location, .. } => {
                (Some(function_name.clone()), source_location.clone())
            },
            causality_engine::ExecutionEvent::FunctionReturn { function_name, .. } => {
