// Test utilities for operations
// Original file: src/operation/test_fixtures.rs
...
mod transformation_tests {
    ...
    fn test_abstract_to_register_transformation() {
        ...
        let register_op = transformation::transform_abstract_to_register(
            &abstract_op,
            register_context
        ).unwrap();
...
mod execution_tests {
    ...
    impl OperationExecutor<AbstractContext> for TestAbstractExecutor {
        async fn execute(&self, operation: &Operation<AbstractContext>) 
            -> std::result::Result<OperationResult<AbstractContext>, ExecutionError> {
