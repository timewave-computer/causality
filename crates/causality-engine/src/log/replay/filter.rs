...
use causality_engine::{LogEntry, EntryType};
    ...
    pub fn matches(&self, entry: &LogEntry) -> bool {
        ...
        if let Some(resources) = &self.resources {
            let entry_resources = match &entry.data {
                causality_engine::EntryData::Effect(effect) => &effect.resources,
                causality_engine::EntryData::Fact(fact) => &fact.resources,
                causality_engine::EntryData::Event(event) => {
        ...
        if let Some(domains) = &self.domains {
            match &entry.data {
                causality_engine::EntryData::Effect(effect) => {
                    if !effect.domains.iter().any(|d| domains.contains(d)) {
                        return false;
                    }
                }
                causality_engine::EntryData::Fact(fact) => {
...
mod tests {
    ...
    use causality_crypto::ContentId;
    use causality_engine::{EntryData, EventEntry, EventSeverity};
