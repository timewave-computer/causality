// State transition effect templates
// Original file: src/effect/templates/state_transition.rs
...
use causality_resource::{RelationshipTracker, RelationshipType};
...
mod tests {
    ...
    async fn test_validate_parent_child_relationship() -> Result<()> {
        ...
        let parent_id = ContentId::from("parent".as_bytes());
        let child_id = ContentId::from("child".as_bytes());
    ...
    async fn test_validate_dependency_relationship() -> Result<()> {
        ...
        let dependent_id = ContentId::from("dependent".as_bytes());
        let dependency_id = ContentId::from("dependency".as_bytes());
