...
// Original file: src/effect/storage.rs
...
use causality_domain_evm::storage_strategy::EthereumStorageEffectFactory;
use causality_domain_cosmwasm::storage_strategy::CosmWasmStorageEffectFactory;
use causality_domain_cosmwasm::CosmWasmAdapterConfig;
use std::str::FromStr;
...
impl Effect for ReadFromChainEffect {
    ...
    async fn execute(&self, context: EffectContext) -> EffectResult<EffectOutcome> {
        ...
        let mock_register = ResourceRegister::new(
            ...
            causality_resource::ResourceLogic::Fungible,
            causality_resource::FungibilityDomain("ETH".to_string()),
            causality_resource::Quantity(100),
            Default::default(),
...
/// Effect for transferring a ResourceRegister across domains
...
impl CrossDomainTransferEffect {
    /// Create a new effect to transfer a ResourceRegister across domains
    pub fn new(
        source_content_id: ContentId,
        target_domain_id: DomainId,
        controller_label: Option<String>,
        ...
        Self {
            ...
            source_domain_id,
            invoker,
            display_name: "Cross-Domain ResourceRegister Transfer".to_string(),
        }
    }
}
...
impl Effect for CrossDomainTransferEffect {
    ...
    fn description(&self) -> &str {
        "Transfers a ResourceRegister across domains"
    }
    
    fn required_capabilities(&self) -> Vec<(ContentId, Right)> {
        // Need write capability on the source register
        vec![(self.source_content_id.clone(), Right::Write)]
    }
    
    async fn execute(&self, context: EffectContext) -> EffectResult<EffectOutcome> {
        // In a real implementation, this would:
        // 1. Retrieve the source ResourceRegister
        // 2. Create a new ResourceRegister on the target domain
        // 3. Mark the source ResourceRegister as consumed
        // 4. Set up cross-domain relationship between the two ResourceRegisters
        ...
        let result = serde_json::json!({
            ...
            "status": "transferred",
            "transaction_id": format!("tx-transfer-{}-{}", self.source_domain_id, self.target_domain_id),
        });
        
        // Track resource changes
        let resource_change_source = ResourceChange {
            resource_id: self.source_content_id.clone(),
            change_type: ResourceChangeType::Consumed,
            previous_state_hash: Some("previous-hash".to_string()),
            new_state_hash: "consumed-hash".to_string(),
        };
        
        // Generate a predictable target ID based on source ID and target domain
        // In a real implementation, this would be properly derived or generated
        let target_content_id = ContentId::from(format!(
            "{}-{}", 
            self.source_content_id, 
            self.target_domain_id
        ));
        
        let resource_change_target = ResourceChange {
            resource_id: target_content_id,
        ...
        Ok(EffectOutcome {
            ...
            result: Some(result),
            error: None,
            resource_changes: vec![resource_change_source, resource_change_target],
            metadata: Default::default(),
        })
    }
    
    fn can_execute_in(&self, boundary: ExecutionBoundary) -> bool {
        // Cross-domain transfers must execute outside the system
        boundary == ExecutionBoundary::OutsideSystem
    }
...
/// Factory function to create a cross-domain transfer effect
pub fn create_cross_domain_transfer_effect(
    source_content_id: ContentId,
    target_domain_id: DomainId,
    controller_label: Option<String>,
    source_domain_id: DomainId,
    ...
    Arc::new(CrossDomainTransferEffect::new(
        source_content_id,
        target_domain_id,
        controller_label,
        source_domain_id,
