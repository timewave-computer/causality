...
pub trait DeFiPrimitive:
    ContentAddressable + Serialize + for<'de> Deserialize<'de> + Clone
{
...
pub enum AssetType {
    ...
    Wrapped {
        chain: String,
        contract: String,
        token_type: Box<AssetType>,
    },

    /// LP token for DEX liquidity
    LiquidityToken {
        pool_id: String,
        token_a: Box<AssetType>,
        token_b: Box<AssetType>,
    },

    /// Collateral representation for lending
    Collateral {
        underlying: Box<AssetType>,
...
pub fn check_slippage(
    ...
    if actual < expected.saturating_sub(max_deviation)
        || actual > expected + max_deviation
    {
