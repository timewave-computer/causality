// Memory-based resource API
// Original file: src/resource/memory_api.rs
...
impl ContentAddressed for ContentIdContentData {
    ...
    fn to_bytes(&self) -> Result<Vec<u8>> {
        let bytes = borsh::to_vec(self)
            .map_err(|e| causality_types::Error::Serialization(format!("Failed to serialize ContentIdContentData: {}", e)))?;
        Ok(bytes)
    }
    
    fn from_bytes(bytes: &[u8]) -> Result<Self> {
        borsh::from_slice(bytes)
            .map_err(|e| causality_types::Error::Deserialization(format!("Failed to deserialize ContentIdContentData: {}", e)))
    }
}
...
impl ResourceAPI for MemoryResourceAPI {
    async fn create_resource(
        ...
        // Create metadata
        let now = causality_resource::unix_timestamp_now();
    ...
    async fn update_resource(
        ...
        if let Some(new_data) = data {
            ...
            resource.metadata.updated_at = causality_resource::unix_timestamp_now();
