...
impl StorageLayoutGenerator {
    ...
    fn generate_contract_layout(
        ...
        for query in queries {
            let (entry, type_def) =
                self.create_storage_entry(query, &mut slot_counter)?;
            storage_entries.push(entry);

            // Add type definition if not already present
            if !type_definitions
                .iter()
                .any(|t: &TypeInfo| t.label == type_def.label)
            {
        ...
        let layout_commitment = self.generate_layout_commitment(
            contract_id,
            &storage_entries,
            &type_definitions,
        )?;
    ...
    fn determine_type_info(&self, query_type: &QueryType) -> (String, TypeInfo) {
        match query_type {
            QueryType::TokenBalance => (
                "t_mapping_address_uint256".to_string(),
                TypeInfo {
            ...
            QueryType::TokenAllowance => (
                "t_mapping_address_mapping_address_uint256".to_string(),
                TypeInfo {
    ...
    #[cfg(feature = "traverse")]
    fn convert_to_traverse_layout(
        &self,
        layout: &StorageLayout,
    ) -> Result<TraverseLayoutInfo> {
        let storage_entries: Vec<TraverseStorageEntry> = layout
            .storage
            .iter()
        ...
        let type_infos: Vec<TraverseTypeInfo> = layout
            .types
            .iter()
    ...
    pub fn validate_layout_commitment(
        ...
        // Validate that layout commitment matches actual layout
        let expected_commitment = self.generate_layout_commitment(
    ...
    /// Convert storage layout to traverse layout info
...
mod tests {
    ...
    fn test_storage_layout_generation() {
        ...
        let layout = generator
            .generate_contract_layout("usdc", &queries)
            .unwrap();
    ...
    fn test_type_mapping() {
        ...
        let (type_name, type_info) =
            generator.determine_type_info(&QueryType::TokenBalance);
    ...
    fn test_traverse_compatibility() {
        ...
        let queries = vec![create_test_query(
            "test",
            "value",
            QueryType::StorageSlot("value".to_string()),
        )];

        let layout = generator
            .generate_contract_layout("test", &queries)
            .unwrap();
