...
use traverse_core::{LayoutInfo as TraverseLayoutInfo, KeyResolver, StaticKeyPath};
...
impl TraverseAlmanacIntegrator {
    ...
    pub async fn generate_witness(&mut self, request: WitnessGenerationRequest) -> Result<WitnessGenerationResult, IntegrationError> {
        ...
        let path_resolution_time = path_resolution_start.elapsed().as_millis() as u64;
        
        // Query state using Almanac
        let almanac_query_start = std::time::Instant::now();
        let witness = self.query_state_for_witness(&request, &storage_path).await?;
    ...
    /// Resolve storage path using Traverse
    async fn resolve_storage_path(&self, request: &WitnessGenerationRequest) -> Result<String, IntegrationError> {
        #[cfg(feature = "traverse")]
        {
            if let Some(ref resolver) = self.key_resolver {
                ...
                let traverse_layout = self.layout_generator.to_traverse_layout(&layout);
                
                // Resolve the query to a storage path
                let static_path = resolver.resolve(&traverse_layout, &request.query)
                    .map_err(|e| IntegrationError::PathResolutionFailed(format!("{:?}", e)))?;
    ...
    async fn query_state_for_witness(&self, request: &WitnessGenerationRequest, storage_path: &str) -> Result<WitnessData, IntegrationError> {
        #[cfg(feature = "almanac")]
        {
            if let Some(ref runtime) = self.almanac_runtime {
                // Query the storage value from Almanac
                let storage_value = runtime.get_storage_value(
                    &request.contract_address,
                    storage_path,
                    request.block_number,
                ).await
                .map_err(|e| IntegrationError::AlmanacQueryFailed(format!("{:?}", e)))?;
                
                // Get the merkle proof for the storage slot
                let merkle_proof = runtime.get_storage_proof(
                    &request.contract_address,
                    storage_path,
                    request.block_number,
                ).await
                .map_err(|e| IntegrationError::AlmanacQueryFailed(format!("Proof generation failed: {:?}", e)))?;
    ...
    fn get_cached_witness(&self, request: &WitnessGenerationRequest) -> Option<CachedWitness> {
        ...
        if let Some(cached) = self.witness_cache.get(&cache_key) {
            ...
            if now - cached.cached_at < self.config.cache_ttl_seconds {
                ...
                if cached.layout_commitment == request.layout_commitment.commitment_hash {
    ...
    /// Cache witness data
    fn cache_witness(&mut self, request: &WitnessGenerationRequest, witness: &WitnessData) {
        let cache_key = self.generate_cache_key(request);
        let now = std::time::SystemTime::now()
            .duration_since(std::time::UNIX_EPOCH)
            .unwrap()
    ...
    /// Get contract layout
    fn get_contract_layout(&self, contract_id: &str) -> Result<StorageLayout, IntegrationError> {
    ...
    fn parse_simple_query(&self, query: &str) -> Result<String, IntegrationError> {
        ...
        Err(IntegrationError::PathResolutionFailed(
            format!("Cannot parse query without Traverse: {}", query)
        ))
    }
    
    /// Validate layout commitment consistency
    pub fn validate_layout_commitment(&self, request: &WitnessGenerationRequest) -> Result<bool, IntegrationError> {
        if self.config.enable_layout_validation {
            let layout = self.get_contract_layout(&request.contract_id)?;
            
            #[cfg(feature = "traverse")]
            {
                return self.layout_generator.validate_layout_commitment(&layout)
            ...
            Ok(layout.layout_commitment.commitment_hash == request.layout_commitment.commitment_hash)
        } else {
            Ok(true)
        }
    }
    ...
    pub fn get_cache_stats(&self) -> CacheStats {
        ...
        let total_usage: u64 = self.witness_cache.values().map(|w| w.usage_count).sum();
