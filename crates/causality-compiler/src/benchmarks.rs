...
impl CompilerBenchmark {
    ...
    pub fn run_all_benchmarks(&mut self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        self.benchmark_source("alloc_simple", "(alloc TokenA 100)")?;
        ...
        self.benchmark_source(
            "alloc_with_bind",
            "(bind (alloc TokenA 42) (pure 100))",
        )?;

        self.benchmark_source(
            "resource_allocation",
            "(alloc TokenA (alloc TokenB 100))",
        )?;

        // Realistic examples using only supported constructs
        self.benchmark_source(
            "token_creation",
            "(bind (pure 1000) (alloc TokenA 100))",
        )?;
        ...
        self.benchmark_source(
            "resource_bind_chain",
            "(bind (alloc TokenA 1) (bind (alloc TokenB 2) (bind (alloc TokenC 3) (pure 42))))",
        )?;
    ...
    pub fn generate_report(&self) -> String {
        ...
        report
            .push_str("=== Causality Compiler Performance Benchmark Report ===\n\n");
        ...
        for result in &self.results {
            report.push_str(&format!(
                ...
                result.source_size,
                result.memory_usage,
                result.instruction_count as f64
                    / result.compilation_time.as_millis().max(1) as f64,
        ...
        if !self.results.is_empty() {
            let total_time: Duration =
                self.results.iter().map(|r| r.compilation_time).sum();
            let avg_instruction_count = self
                .results
                .iter()
                .map(|r| r.instruction_count)
                .sum::<usize>()
                / self.results.len();
            let avg_gas = self.results.iter().map(|r| r.gas_estimate).sum::<u64>()
...
fn count_unique_registers(
    instructions: &[causality_core::machine::Instruction],
) -> usize {
...
mod tests {
    ...
    fn test_benchmark_simple_compilation() {
        ...
        let result = benchmark
            .benchmark_source("test_simple", "(pure 42)")
            .unwrap();
    ...
    fn test_benchmark_alloc_performance() {
        let mut benchmark = CompilerBenchmark::new();
        let result = benchmark
            .benchmark_source("test_alloc", "(alloc TokenA 100)")
            .unwrap();
    ...
    fn test_full_benchmark_suite() {
        ...
        let test_names: Vec<_> = benchmark
            .results()
            .iter()
    ...
    fn test_performance_report_generation() {
        let mut benchmark = CompilerBenchmark::new();
        benchmark
            .benchmark_source("test_report", "(pure 42)")
            .unwrap();
    ...
    fn test_performance_targets() {
        ...
        benchmark
            .benchmark_source("pure_literal", "(pure 42)")
            .unwrap();
    ...
    fn test_instruction_efficiency() {
        ...
        let simple_result =
            benchmark.benchmark_source("simple", "(pure 42)").unwrap();
        let complex_result = benchmark
            .benchmark_source(
                "complex",
                "(bind (pure 10) (bind (pure 20) (pure 30)))",
            )
    ...
    fn test_compilation_scaling() {
        ...
        let medium = benchmark
            .benchmark_source("medium", "(bind (pure 1) (bind (pure 2) (pure 3)))")
            .unwrap();
        let large = benchmark.benchmark_source("large",
            "(bind (alloc TokenA 1) (bind (alloc TokenB 2) (bind (alloc TokenC 3) (bind (alloc TokenD 4) (pure 5)))))").unwrap();
    ...
    fn test_register_usage_efficiency() {
        ...
        let simple = benchmark
            .benchmark_source("simple_reg", "(pure 42)")
            .unwrap();
        let complex = benchmark
            .benchmark_source(
                "complex_reg",
                "(bind (alloc TokenA 1) (bind (alloc TokenB 2) (bind (alloc TokenC 3) (pure 42))))",
            )
