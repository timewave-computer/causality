            ...
            match &exprs[0] {
                SExpression::Symbol(op) => match op.as_str() {
                    ...
                    "alloc" => {
                        ...
                        let _resource_type = check_sexpr_with_env(&exprs[1], env)?;
                        let _value_type = check_sexpr_with_env(&exprs[2], env)?;
                    ...
                    "record-get" => {
                        ...
                        let field_name = match &exprs[2] {
                            ...
                            _ => {
                                return Err(CompileError::TypeError {
                                    message: "record-get requires a field name"
                                        .to_string(),
                        ...
                        let required_cap =
                            Capability::read_field("record_access", &field_name);
                        if !env.has_capability(&required_cap) {
                            return Err(CompileError::TypeError {
                                message: format!(
                                    "Missing capability to read field '{}'",
                                    field_name
                                ),
                        ...
                        match record_type {
                            TypeInner::Record(record_type) => {
                                match record_type.row.project(&field_name) {
                                    RowOpResult::Success(field_type) => {
                                        Ok(field_type)
                                    }
                            ...
                            _ => Err(CompileError::TypeError {
                                message: "record-get requires a record type"
                                    .to_string(),
                    ...
                    "record-set" => {
                        ...
                        let field_name = match &exprs[2] {
                            ...
                            _ => {
                                return Err(CompileError::TypeError {
                                    message: "record-set requires a field name"
                                        .to_string(),
                                    expected: Some("Symbol".to_string()),
                                    found: Some(format!("{:?}", exprs[2])),
                                    location: None,
                        ...
                        let required_cap =
                            Capability::write_field("record_access", &field_name);
                        if !env.has_capability(&required_cap) {
                            return Err(CompileError::TypeError {
                                message: format!(
                                    "Missing capability to write field '{}'",
                                    field_name
                                ),
...
fn check_linearity_with_tracker(
    expr: &SExpression,
    tracker: &mut LinearityTracker,
) -> CompileResult<()> {
    match expr {
        SExpression::Integer(_)
        | SExpression::Boolean(_)
...
pub fn check_capability_access(
    ...
    let required_cap = match (operation, field) {
        ("read", Some(field_name)) => {
            Capability::read_field("operation", field_name)
        }
        ("write", Some(field_name)) => {
            Capability::write_field("operation", field_name)
        }
        ("execute", _) => Capability::new(
            "operation",
            causality_core::effect::CapabilityLevel::Execute,
        ),
        _ => {
            return Err(CompileError::TypeError {
...
mod tests {
    ...
    fn test_check_alloc_expression() {
        let expr = SExpression::List(vec![
            ...
            SExpression::Integer(42), // Use a literal instead of a symbol
    ...
    fn test_record_access_with_capability() {
        use causality_core::effect::{FieldType, RecordType, RowType};
        use std::collections::BTreeMap;
    ...
    fn test_record_write_missing_capability() {
        ...
        assert!(
            result.is_err(),
            "Expected error for missing write capability, but got: {:?}",
            result
        );
        match result.unwrap_err() {
            CompileError::TypeError { message, .. } => {
            ...
            err => panic!(
                "Expected TypeError for missing write capability, but got: {:?}",
                err
            ),
        }
    }
    ...
    fn test_row_type_zero_runtime_overhead() {
        use causality_core::effect::{FieldType, RowType};
        use std::collections::BTreeMap;
        ...
        fields.insert(
            "x".to_string(),
            FieldType::simple(TypeInner::Base(BaseType::Int)),
        );
        fields.insert(
            "y".to_string(),
            FieldType::simple(TypeInner::Base(BaseType::Int)),
