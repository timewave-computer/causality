...
use crate::traverse_almanac_integration::{
    IntegrationError, TraverseAlmanacIntegrator, WitnessGenerationRequest,
};
use anyhow::Result;
...
impl ProofPrimitiveCompiler {
    ...
    pub fn register_storage_layout(
        &mut self,
        contract_id: String,
    ...
    pub fn compile_prove_state(
        &self,
        expr: &Expr,
    ) -> Result<CompiledProof, ProofCompileError> {
    ...
    fn extract_proof_primitive(
        &self,
        expr: &Expr,
    ) -> Result<ProveStatePrimitive, ProofCompileError> {
        match &expr.kind {
    ...
    fn parse_prove_state_args(
        &self,
        args: &[Expr],
    ) -> Result<ProveStatePrimitive, ProofCompileError> {
        if args.len() < 2 {
            return Err(ProofCompileError::InvalidArguments(
                "prove_state requires at least contract_id and storage_slot"
                    .to_string(),
            ));
        }
    ...
    fn get_storage_layout(
        &self,
        contract_id: &str,
    ) -> Result<&StorageLayout, ProofCompileError> {
        self.storage_layouts.get(contract_id).ok_or_else(|| {
            ProofCompileError::StorageLayoutNotFound(contract_id.to_string())
        })
    }
    ...
    ) -> Result<WitnessData, ProofCompileError> {
    ...
    pub async fn generate_witness_data_async(
        &mut self,
        primitive: &ProveStatePrimitive,
        _layout: &StorageLayout,
        block_number: u64,
        ...
        match self.integrator.generate_witness(request).await {
            ...
            Err(e) => Err(ProofCompileError::WitnessGenerationFailed(format!(
                "{:?}",
                e
            ))),
        }
    ...
    fn generate_proof_config(
        &self,
        primitive: &ProveStatePrimitive,
    ) -> Result<ProofGenerationConfig, ProofCompileError> {
    ...
    fn extract_string_literal(
        &self,
        expr: &Expr,
    ) -> Result<String, ProofCompileError> {
        match &expr.kind {
            ExprKind::Const(LispValue::String(s)) => Ok(s.to_string()),
            _ => Err(ProofCompileError::InvalidArguments(
...
mod tests {
    ...
    fn test_proof_type_inference() {
        ...
        assert!(matches!(
            compiler.infer_proof_type("balances"),
            ProofType::BalanceProof
        ));
        assert!(matches!(
            compiler.infer_proof_type("allowances"),
            ProofType::AllowanceProof
        ));
        assert!(matches!(
            compiler.infer_proof_type("other"),
            ProofType::StorageInclusion
        ));
    }
}
