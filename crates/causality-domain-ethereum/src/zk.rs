// Zero-knowledge proof integration for EVM
// Original file: src/domain_adapters/evm/zk.rs
...
use causality_crypto::{ContentAddressed, ContentId, HashOutput, HashFactory, HashError, HashAlgorithm};
...
impl ContentAddressed for EvmZkProveEffect {
    fn content_hash(&self) -> Result<ContentId, HashError> {
        // Use standardized approach by serializing and hashing
        let bytes = self.to_bytes()?;
        let hash_output = HashOutput::from_bytes(&bytes, HashAlgorithm::Blake3)?;
        Ok(ContentId::from(hash_output))
    }
    
    fn verify(&self, content_id: &ContentId) -> Result<bool, HashError> {
        let computed_id = self.content_hash()?;
        Ok(computed_id == *content_id)
    }
