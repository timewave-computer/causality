...
// Original file: src/domain_adapters/evm/adapter.rs
...
use causality_engine_types::FactType; // Import the new FactType enum
use causality_types::{Error, Result};
use crate::util::to_fixed_bytes;
use causality_core::effect::{Effect, EffectContext, EffectError, EffectOutcome, EffectHandler, EffectTypeId, HandlerResult, DowncastEffect};
use causality_domain_evm::zk::{
...
impl EthereumAdapter {
    ...
    async fn handle_register_create_query(&self, query: &FactQuery) -> Result<FactType> {
        ...
        let fact = FactType::RegisterFact(causality_engine_types::RegisterFact::RegisterCreation {
            domain_id: self.config.domain_id.clone(),
    ...
    async fn handle_register_update_query(&self, query: &FactQuery) -> Result<FactType> {
        ...
        let fact = FactType::RegisterFact(causality_engine_types::RegisterFact::RegisterUpdate {
            domain_id: self.config.domain_id.clone(),
            register_id: register_id.to_string(),
    ...
    async fn handle_register_transfer_query(&self, query: &FactQuery) -> Result<FactType> {
        ...
        let fact = FactType::RegisterFact(causality_engine_types::RegisterFact::RegisterTransfer {
            domain_id: self.config.domain_id.clone(),
            register_id: register_id.to_string(),
            from: from.to_string(),
            to: to.to_string(),
...
impl EffectHandler for EthereumAdapter {
    // Renamed from execute_async and updated context type
    async fn handle(&self, effect: &dyn Effect, context: &dyn EffectContext) -> HandlerResult<EffectOutcome> {
        match effect.name() {
            "evm_zk_compile" => {
                // Use downcast_ref from DowncastEffect trait (assuming Effect implements it)
    ...
    // Renamed from can_handle and updated return type
    fn supported_effect_types(&self) -> Vec<EffectTypeId> {
        vec![
            EffectTypeId::from_str("evm_zk_compile").unwrap(),
            EffectTypeId::from_str("evm_zk_witness").unwrap(),
            EffectTypeId::from_str("evm_zk_prove").unwrap(),
            EffectTypeId::from_str("evm_zk_verify").unwrap(),
        ]
    }
}
    ...
    async fn test_register_facts() {
        ...
        let create_result = match adapter.handle_register_create_query(&register_create_query).await {
            ...
            Err(e) => {
                ...
                FactType::RegisterFact(causality_engine_types::RegisterFact::RegisterCreation {
                    domain_id: adapter.domain_id().clone(),
        ...
        let update_result = match adapter.handle_register_update_query(&register_update_query).await {
            ...
            Err(e) => {
                ...
                FactType::RegisterFact(causality_engine_types::RegisterFact::RegisterUpdate {
                    domain_id: adapter.domain_id().clone(),
                    register_id: "test-register-1".to_string(),
        ...
        let transfer_result = match adapter.handle_register_transfer_query(&register_transfer_query).await {
            ...
            Err(e) => {
                ...
                FactType::RegisterFact(causality_engine_types::RegisterFact::RegisterTransfer {
                    domain_id: adapter.domain_id().clone(),
                    register_id: "test-register-1".to_string(),
