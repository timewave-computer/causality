...
let test_machine_creation () =
  ...
  let unit_val = Value.RegisterValue.create_unrestricted (Value.Primitive Value.Unit) in
  let _state_with_reg = State.MachineState.set_register state (Instruction.RegisterId.create 0l) unit_val in
...
let test_single_step () =
  ...
  (* Set up register 0 with a unit value *)
  let unit_val = Value.RegisterValue.create_unrestricted (Value.Primitive Value.Unit) in
  let state = State.MachineState.set_register state (Instruction.RegisterId.create 0l) unit_val in
  ...
  | Ok new_state ->
      ...
      (match State.MachineState.get_register new_state (Instruction.RegisterId.create 1l) with
       | None -> 
...
let test_constraint_evaluation () =
  ...
  let int_val1 = Value.RegisterValue.create_unrestricted (Value.Primitive (Value.Int 5)) in
  let int_val2 = Value.RegisterValue.create_unrestricted (Value.Primitive (Value.Int 10)) in
  let state = State.MachineState.set_register state (Instruction.RegisterId.create 1l) int_val1 in
  let state = State.MachineState.set_register state (Instruction.RegisterId.create 2l) int_val2 in
  
  (* Test various constraints *)
  let constraint_true = Instruction.True in
  let constraint_false = Instruction.False in
  let constraint_less = Instruction.LessThan (Instruction.RegisterId.create 1l, Instruction.RegisterId.create 2l) in
  let constraint_greater = Instruction.GreaterThan (Instruction.RegisterId.create 1l, Instruction.RegisterId.create 2l) in
  
  assert (Reduction.eval_constraint state constraint_true = true);
  assert (Reduction.eval_constraint state constraint_false = false);
  assert (Reduction.eval_constraint state constraint_less = true);   (* 5 < 10 *)
  assert (Reduction.eval_constraint state constraint_greater = false); (* 5 > 10 is false *)
...
let test_effect_execution () =
  ...
  | Ok new_state ->
      ...
      (match State.MachineState.get_register new_state (Instruction.RegisterId.create 1l) with
       | None -> 
           Printf.printf "✗ Effect execution test failed: output register not found\n";
           assert false
       | Some reg_val ->
           assert (reg_val.value = Value.Primitive Value.Unit);
...
let test_resource_allocation () =
  ...
  let type_val = Value.RegisterValue.create_unrestricted (Value.Primitive (Value.Symbol "TestType")) in
  let val_val = Value.RegisterValue.create_unrestricted (Value.Primitive (Value.Int 42)) in
  let state = State.MachineState.set_register state (Instruction.RegisterId.create 0l) type_val in
  let state = State.MachineState.set_register state (Instruction.RegisterId.create 1l) val_val in
  ...
  | Ok state_after_alloc ->
      ...
      (match State.MachineState.get_register state_after_alloc (Instruction.RegisterId.create 2l) with
       | None -> 
           Printf.printf "✗ Resource allocation test failed: resource not created\n";
           assert false
       | Some reg_val ->
           (match reg_val.value with
            | Value.ResourceRef _ ->
                ...
                (match Reduction.step state_after_alloc with
                 | Error err -> 
                     Printf.printf "✗ Resource allocation test failed at consume: %s\n" (string_of_error_kind err);
                     assert false
                 | Ok state_after_consume ->
                     (* Check that consumption result was created *)
                     (match State.MachineState.get_register state_after_consume (Instruction.RegisterId.create 3l) with
                      | None -> 
                          Printf.printf "✗ Resource allocation test failed: consumption result not found\n";
                          assert false
                      | Some _result_val ->
                          Printf.printf "✓ Resource allocation and consumption test passed\n"))
            | _ -> 
                Printf.printf "✗ Resource allocation test failed: not a resource reference\n";
                assert false))
...
let test_debugging () =
  ...
  assert (String.contains debug_output '0'); (* Should contain PC = 0 *)
  
  let reg_output = Reduction.debug_registers state in
  assert (String.contains reg_output 'R'); (* Should contain register info *)
