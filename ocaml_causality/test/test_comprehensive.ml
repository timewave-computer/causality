...
let () =
  ...
  (match id1, id2, id3 with
   | Some id1, Some id2, Some id3 ->
     if EntityId.equal id1 id2 then
       Printf.printf "✓ Identical content produces identical EntityIds at Layer 0\n"
     else
       Printf.printf "✗ ERROR: Identical content should produce identical EntityIds\n";
     
     if not (EntityId.equal id1 id3) then
       Printf.printf "✓ Different content produces different EntityIds at Layer 0\n"
     else
       Printf.printf "✗ ERROR: Different content should produce different EntityIds\n"
  ...
  if EntityId.equal int_expr1 int_expr2 then
    ...
    Printf.printf "✗ ERROR: Identical expressions should produce identical ExprIds\n";
  
  if not (EntityId.equal int_expr1 int_expr3) then
    Printf.printf "✓ Different expressions produce different ExprIds\n"
  else
    Printf.printf "✗ ERROR: Different expressions should produce different ExprIds\n";
  ...
  let complex_expr1 = Term.Term.if_then_else store bool_expr int_expr1 symbol_expr in
  let complex_expr2 = Term.Term.if_then_else store bool_expr int_expr1 symbol_expr in
  let complex_expr3 = Term.Term.if_then_else store bool_expr int_expr3 symbol_expr in
  ...
  if EntityId.equal complex_expr1 complex_expr2 then
    ...
    Printf.printf "✗ ERROR: Identical complex expressions should produce identical ExprIds\n";
  ...
  if not (EntityId.equal complex_expr1 complex_expr3) then
    ...
    Printf.printf "✗ ERROR: Different complex expressions should produce different ExprIds\n";
  ...
  let app_expr = Term.CorePrimitives.function_elim store lambda_expr int_expr2 in
  
  Printf.printf "Unit expression: %s (ID: %s)\n" 
    (Term.Pretty.term_to_string store unit_expr) (EntityId.to_hex unit_expr);
  ...
  let tensor_expr = Term.CorePrimitives.product_intro store int_expr1 symbol_expr in
  let tensor_elim = Term.CorePrimitives.product_elim store tensor_expr unit_expr in
  
  Printf.printf "Tensor expression: %s (ID: %s)\n" 
    (Term.Pretty.term_to_string store tensor_expr) (EntityId.to_hex tensor_expr);
  ...
  let case_expr = Term.CorePrimitives.sum_elim store left_expr int_expr2 symbol_expr in
  ...
  let expr_a = Term.Term.if_then_else store bool_expr shared_subexpr unit_expr in
  let expr_b = Term.Term.if_then_else store bool_expr shared_subexpr unit_expr in
  let expr_c = Term.Term.let_bind store "x" shared_subexpr unit_expr in
  
  if EntityId.equal expr_a expr_b then
    Printf.printf "✓ Expressions with shared subexpressions have identical IDs\n"
  else
    Printf.printf "✗ ERROR: Expressions with shared subexpressions should have identical IDs\n";
  
  if not (EntityId.equal expr_a expr_c) then
    Printf.printf "✓ Different expression structures have different IDs even with shared subexpressions\n"
  else
    Printf.printf "✗ ERROR: Different expression structures should have different IDs\n";
  
  Printf.printf "\n=== Content Store Statistics ===\n";
  Printf.printf "Note: In a production system, the content store would provide statistics\n";
  Printf.printf "about storage efficiency, deduplication rates, and cache hit ratios.\n";
