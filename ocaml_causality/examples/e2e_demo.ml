...
let demo_linear_resources () =
  ...
  let _ = match Ffi.initialize_ffi () with
    | Ok () -> Printf.printf "FFI initialized successfully\n"
    | Error err -> Printf.printf "FFI initialization failed: %s\n" 
        (match err with 
         | FFIError msg -> msg 
         | _ -> "Unknown error")
  in
  ...
  let create_resource name quantity =
    ...
    | Ok (Some resource_id) -> 
        Printf.printf "Created resource '%s' with quantity %Ld: %s\n" 
          name quantity (Bytes.to_string resource_id);
        Some resource_id
    | Ok None -> 
  ...
  (match token_id with
   | Some id -> 
       (match Ffi.safe_consume_resource_by_id id with
        ...
        | Error err -> Printf.printf "Error consuming resource: %s\n"
            (match err with 
  ...
  (match token_id with
   | Some id -> 
       (match Ffi.safe_consume_resource_by_id id with
        ...
        | Error (InvalidResource _) -> Printf.printf "Correctly prevented double consumption (resource invalid)\n"
...
let demo_expressions () =
  ...
  let lambda_expr = Expr.lambda 
    ...
    (Expr.apply (Expr.const (LispValue.symbol "+")) 
                [Expr.const (LispValue.symbol "x"); 
                 Expr.const (LispValue.symbol "y")]) in
  ...
  (match Expr.get_predefined_expr_id "issue_ticket_logic" with
   | Some expr_id -> Printf.printf "Found predefined expression 'issue_ticket_logic': %s\n" 
       (Bytes.to_string expr_id)
   | None -> Printf.printf "Predefined expression 'issue_ticket_logic' not found\n")
...
let demo_content_addressing () =
  ...
  (match Content_addressing.retrieve_content store id1 with
   | Some content -> Printf.printf "Retrieved content 1: %s\n" (Bytes.to_string content)
  ...
  List.iter (fun (id, is_valid) ->
    Printf.printf "  %s: %s\n" (Bytes.to_string id) (if is_valid then "VALID" else "INVALID")
  ) integrity_results;
...
let demo_patterns () =
  ...
  Printf.printf "Pattern matching functionality is available but not exposed in this demo.\n";
  Printf.printf "The core system supports resource patterns and flow management.\n"
...
let demo_system_metrics () =
  ...
  | Error err -> Printf.printf "Failed to get metrics: %s\n"
      (match err with 
       | FFIError msg -> msg 
       | _ -> "Unknown error")
...
let run_demo () =
  ...
  Printf.printf "  • Linear resource management with automatic consumption tracking\n";
