...
let () =
  ...
  let issue_ticket_expr = Expr.apply 
    (Expr.const (LispValue.symbol "issue-ticket"))
    [ticket_id_expr; event_name_expr; owner_expr] in
  ...
  (match Expr.get_predefined_expr_id "issue_ticket_logic" with
   ...
   | None -> 
       Printf.printf "Warning: predefined issue ticket logic not found\n");
  ...
  (match Intent.submit issue_intent with
   | Ok () -> 
       ...
       Printf.printf "\n--- Ticket Transfer ---\n";
       let transfer_intent = Intent.create 
         ~name:"TransferTicket" 
         ~domain_id:"Ticketing" in
       
       Intent.add_input_resource transfer_intent new_ticket_resource_id;
       Intent.add_parameter transfer_intent (LispValue.string "another_ocaml_user_pk");
  ...
  (match System.get_resource_by_id (Bytes.of_string "test_resource") with
   | Ok (Some resource) -> 
       Printf.printf "Found resource: %s\n" resource.name
   | Ok None -> 
       Printf.printf "Resource not found\n"
   | Error _err -> 
       Printf.printf "Error getting resource\n");
  ...
  (match System.get_system_metrics () with
   ...
   | Error _err -> 
       Printf.printf "Error getting system metrics\n");
  ...
  (match System.get_domain_info ticketing_domain_id with
   | Ok (Some domain) -> 
       Printf.printf "Domain found: %s\n" 
         (match domain with
          | VerifiableDomain d -> "VerifiableDomain(" ^ Bytes.to_string d.domain_id ^ ")"
          | ServiceDomain d -> "ServiceDomain(" ^ Bytes.to_string d.domain_id ^ ")"
          | ComputeDomain d -> "ComputeDomain(" ^ Bytes.to_string d.domain_id ^ ")")
   | Ok None -> 
       Printf.printf "Domain not found\n"
   | Error _err -> 
       Printf.printf "Error getting domain info\n");
