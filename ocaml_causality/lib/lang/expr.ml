...
module Expr = struct
  ...
  let rec free_variables = function
    ...
    | If (cond, then_expr, else_expr) ->
        List.concat
          [
            free_variables cond
          ; free_variables then_expr
          ; free_variables else_expr
          ]
    | Sequence exprs -> List.concat (List.map free_variables exprs)
  ...
  let empty_context = { bindings = []; resources = [] }

  let bind_value name value ctx =
    { ctx with bindings = (name, value) :: ctx.bindings }

  let lookup_binding name ctx = List.assoc_opt name ctx.bindings
  ...
  let rec eval_expr ctx = function
    ...
    | Lambda (_params, _body) ->
        ...
        Ok
          (LispValue.list
             [ LispValue.symbol "closure"; LispValue.list _params ])
    | Apply (func, _args) -> (
