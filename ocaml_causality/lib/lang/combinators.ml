...
let flip f =
  Expr.lambda
    ...
    (Expr.apply f
       [ Expr.const (LispValue.symbol "y"); Expr.const (LispValue.symbol "x") ])
  ...
  Expr.lambda
    ...
    (Expr.lambda
       ...
       (Expr.apply f
          [
            Expr.const
              (LispValue.list [ LispValue.symbol "x"; LispValue.symbol "y" ])
          ]))

let uncurry f =
  Expr.lambda
    [ LispValue.symbol "pair" ]
    (Expr.let_binding "x"
       (Expr.apply
          (Expr.const (LispValue.symbol "car"))
          [ Expr.const (LispValue.symbol "pair") ])
       (Expr.let_binding "y"
          (Expr.apply
             (Expr.const (LispValue.symbol "cdr"))
             [ Expr.const (LispValue.symbol "pair") ])
          (Expr.apply
             (Expr.apply f [ Expr.const (LispValue.symbol "x") ])
             [ Expr.const (LispValue.symbol "y") ])))
...
let cons_primitive =
  Expr.lambda
    [ LispValue.symbol "head"; LispValue.symbol "tail" ]
    (Expr.apply
       (Expr.const (LispValue.symbol "cons"))
       [
         Expr.const (LispValue.symbol "head")
       ; Expr.const (LispValue.symbol "tail")
       ])
...
let length_primitive =
  Expr.lambda
    [ LispValue.symbol "list" ]
    (Expr.apply
       (Expr.const (LispValue.symbol "length"))
       [ Expr.const (LispValue.symbol "list") ])
...
let partial_apply combinator args = apply_combinator combinator args
...
let map_combinator f =
  Expr.lambda
    [ LispValue.symbol "list" ]
    (Expr.apply
       (Expr.const (LispValue.symbol "map"))
...
let filter_combinator predicate =
  Expr.lambda
    [ LispValue.symbol "list" ]
    (Expr.apply
       (Expr.const (LispValue.symbol "filter"))
...
let fold_combinator f init =
  Expr.lambda
    [ LispValue.symbol "list" ]
    (Expr.apply
       (Expr.const (LispValue.symbol "fold"))
...
module CombinatorRegistry = struct
  ...
  let list_combinators registry = List.map fst !registry
end
