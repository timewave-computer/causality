(** Layer 0: Register Machine Execution Engine
    ...
    This module implements the execution engine for the 11-instruction register
    machine with linearity enforcement and constraint checking. *)
...
let rec eval_constraint (state : machine_state)
    (expr : Instruction.constraint_expr) : bool =
  match expr with
  | True -> true
  | False -> false
  | And (left, right) ->
  ...
  | Or (left, right) ->
      eval_constraint state left || eval_constraint state right
  | Not expr -> not (eval_constraint state expr)
  | Equal (reg1, reg2) -> (
      match
        ( State.MachineState.get_register state reg1
        , State.MachineState.get_register state reg2 )
      with
      | Some r1, Some r2 -> (
  ...
  | HasCapability (reg, capability) -> (
      ...
      match State.MachineState.get_register state reg with
      | Some _ ->
          capability = "read" || capability = "write" (* Simplistic check *)
      | None -> false)
...
let exec_instruction (state : machine_state) (instr : Instruction.instruction) :
  ...
  | Move { src; dst } -> (
      ...
      | Some src_val ->
          ...
          in
          let new_state =
            State.MachineState.set_register new_state dst src_val
          in
          Ok (State.MachineState.advance_pc new_state))
  (* 2. Apply: Function application *)
  | Apply { fn_reg; arg_reg; out_reg } -> (
      match
        ( State.MachineState.get_register state fn_reg
        , State.MachineState.get_register state arg_reg )
      with
      | Some fn_val, Some _arg_val -> (
          ...
          | Primitive (Symbol builtin_name) -> (
              ...
              | "add" | "subtract" | "multiply" | "equal" | "less_than" ->
                  ...
                  in
                  let new_state =
                    State.MachineState.set_register state out_reg result_val
                  in
                  Ok (State.MachineState.advance_pc new_state)
              | _ ->
                  Error
                    (MachineError ("Unknown built-in function: " ^ builtin_name))
              )
  ...
  | Match
      ...
      | Some _sum_val ->
          ...
          in
          let new_state =
            State.MachineState.set_register state left_reg unit_val
          in
          Ok (State.MachineState.advance_pc new_state))
  ...
  | Alloc { type_reg = _type_reg; val_reg; out_reg } -> (
      ...
      | Some val_reg_val ->
          let new_state, resource_id =
            State.MachineState.alloc_resource state val_reg_val.value
          in
          (* Store the resource_id directly by converting to hex *)
          let hex_content = Bytes.to_string resource_id in
          let _entity_id =
            Causality_system.System_content_addressing.EntityId.from_content
              hex_content
          in
          let resource_val =
            Value.RegisterValue.create_linear (ResourceRef _entity_id)
          in
          let new_state =
            State.MachineState.set_register new_state out_reg resource_val
          in
          Ok (State.MachineState.advance_pc new_state))
  ...
  | Consume { resource_reg; out_reg } -> (
      ...
      | Some resource_val -> (
          ...
          | ResourceRef _entity_id -> (
              ...
              match
                State.MachineState.consume_resource state dummy_resource_id
              with
              | Error _ ->
              ...
              | Ok (new_state, consumed_value) ->
                  ...
                  in
                  let new_state =
                    State.MachineState.set_register new_state out_reg result_val
                  in
                  Ok (State.MachineState.advance_pc new_state))
  ...
  | Check { expr } ->
      ...
      else Error (MachineError "Constraint check failed")
  ...
  | Perform { effect; out_reg } ->
      ...
      if not (eval_constraint state effect.pre) then
        ...
        if eval_constraint new_state effect.post then
          Ok (State.MachineState.advance_pc new_state)
        else Error (MachineError "Effect postcondition failed")
  ...
  | Select { cond_reg; true_reg; false_reg; out_reg } -> (
      ...
      | Some cond_val -> (
          ...
          | Primitive (Bool true) -> (
              ...
              | Some true_val ->
                  let new_state =
                    State.MachineState.set_register state out_reg true_val
                  in
                  Ok (State.MachineState.advance_pc new_state))
          | Primitive (Bool false) -> (
              ...
              | Some false_val ->
                  let new_state =
                    State.MachineState.set_register state out_reg false_val
                  in
                  Ok (State.MachineState.advance_pc new_state))
  ...
  | Witness { out_reg } ->
      ...
      let witness_val =
        Value.RegisterValue.create_unrestricted
          (Primitive (Symbol "witness_placeholder"))
      in
      let new_state =
        State.MachineState.set_register state out_reg witness_val
      in
      Ok (State.MachineState.advance_pc new_state)
