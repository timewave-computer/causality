...
module ResourceHeap = struct
  type t = (bytes, machine_value * bool) Hashtbl.t  (* resource_id -> (value, consumed) *)
...
module RegisterFile = struct
  ...
  let consume (registers : t) (reg_id : register_id) : (t, string) result =
    ...
    | Some reg_val when reg_val.metadata.consumed -> Error "Register already consumed"
    | Some reg_val ->
...
module MachineState = struct
  ...
  (** Get register value *)
  let get_register (state : machine_state) (reg_id : register_id) : register_value option =
    RegisterFile.get state.registers reg_id
  
  (** Set register value *)
  let set_register (state : machine_state) (reg_id : register_id) (value : register_value) : machine_state =
    let new_registers = RegisterFile.set state.registers reg_id value in
    { state with registers = new_registers }
  
  (** Allocate resource *)
  let alloc_resource (state : machine_state) (value : machine_value) : machine_state * bytes =
    let (new_heap, resource_id) = ResourceHeap.alloc state.heap value in
    ({ state with heap = new_heap }, resource_id)
  
  (** Consume resource *)
  let consume_resource (state : machine_state) (resource_id : bytes) : (machine_state * machine_value, string) result =
