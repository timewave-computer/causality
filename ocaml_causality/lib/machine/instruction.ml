(** Layer 0: Register Machine Instructions
    ...
    This module implements the 11-instruction register machine that forms
    the verifiable execution core of the Causality framework.
*)
...
type instruction =
  ...
  | Return of { 
      result_reg : RegisterId.t option 
    }
  [@@deriving show, eq]
...
let reads_from (instr : instruction) : RegisterId.t list =
  ...
  | Match { sum_reg; left_reg = _; right_reg = _; left_label = _; right_label = _ } -> [sum_reg]
  | Alloc { type_reg; val_reg; out_reg = _ } -> [type_reg; val_reg]
  | Consume { resource_reg; out_reg = _ } -> [resource_reg]
  | Check _ -> []
  | Perform _ -> []
  | Select { cond_reg; true_reg; false_reg; out_reg = _ } -> [cond_reg; true_reg; false_reg]
  | Witness _ -> []
  | LabelMarker _ -> []
...
let writes_to (instr : instruction) : RegisterId.t list =
  ...
  | Match { sum_reg = _; left_reg; right_reg; left_label = _; right_label = _ } -> [left_reg; right_reg]
...
let is_control_flow (instr : instruction) : bool =
  match instr with
  | Match _ | LabelMarker _ | Return _ -> true
  | _ -> false 