(** Layer 1: Content-Addressed Lambda Calculus Terms
    ...
    This module provides content-addressed linear lambda calculus with the 11
    core primitives and extended AST for practical programming. *)
...
type core_term =
  ...
  | Case of
      expr_id
      * expr_id
      * expr_id (* Pattern matching: case e of inl x => e1 | inr y => e2 *)
  (* Linear functions *)
...
module ExpressionStore = struct
  ...
  let contains (store : t) (id : expr_id) : bool = Hashtbl.mem store id
end
...
module Term = struct
  ...
  let let_unit (store : ExpressionStore.t) (e1 : expr_id) (e2 : expr_id) :
      expr_id =
  ...
  let let_tensor (store : ExpressionStore.t) (e1 : expr_id) (e2 : expr_id) :
      expr_id =
  ...
  let case (store : ExpressionStore.t) (e : expr_id) (e1 : expr_id)
      (e2 : expr_id) : expr_id =
    ExpressionStore.store store (Core (Case (e, e1, e2)))

  let lambda (store : ExpressionStore.t) (var : string) (body : expr_id) :
      expr_id =
  ...
  let if_then_else (store : ExpressionStore.t) (cond : expr_id)
      (then_branch : expr_id) (else_branch : expr_id) : expr_id =
    ExpressionStore.store store (If (cond, then_branch, else_branch))

  let let_bind (store : ExpressionStore.t) (var : string) (value : expr_id)
      (body : expr_id) : expr_id =
  ...
  (** Row type operations *)
  let read_field (store : ExpressionStore.t) (record : expr_id) (field : string)
      : expr_id =
    ExpressionStore.store store (ReadField (record, field))

  let update_field (store : ExpressionStore.t) (record : expr_id)
      (field : string) (value : expr_id) : expr_id =
    ExpressionStore.store store (UpdateField (record, field, value))

  let extend_record (store : ExpressionStore.t) (record : expr_id)
      (field : string) (value : expr_id) : expr_id =
...
module FreeVars = struct
  let rec free_vars (store : ExpressionStore.t) (expr_id : expr_id) :
      string list =
    ...
    | Some expr -> (
        ...
        | Core (Lambda (var, body)) ->
            List.filter
              (fun v -> not (String.equal v var))
              (free_vars store body)
        | Let (var, value, body) ->
...
module Pretty = struct
  let rec term_to_string (store : ExpressionStore.t) (expr_id : expr_id) :
      string =
    match ExpressionStore.retrieve store expr_id with
    ...
    | Some expr -> (
        ...
        | Core Unit -> "()"
        | Core (LetUnit (e1, e2)) ->
            Printf.sprintf "(let () = %s in %s)" (term_to_string store e1)
              (term_to_string store e2)
        | Core (Tensor (e1, e2)) ->
            Printf.sprintf "(%s, %s)" (term_to_string store e1)
              (term_to_string store e2)
        | Core (Lambda (var, body)) ->
            Printf.sprintf "(λ%s. %s)" var (term_to_string store body)
        | Core (Apply (fn, arg)) ->
            Printf.sprintf "(%s %s)" (term_to_string store fn)
              (term_to_string store arg)
        | Symbol s -> s
        | Int i -> string_of_int i
        | Bool b -> string_of_bool b
        | Let (var, value, body) ->
            Printf.sprintf "(let %s = %s in %s)" var
              (term_to_string store value)
              (term_to_string store body)
        | If (cond, then_branch, else_branch) ->
            Printf.sprintf "(if %s then %s else %s)"
              (term_to_string store cond)
              (term_to_string store then_branch)
              (term_to_string store else_branch)
        | _ -> "complex_term")
end
...
module CorePrimitives = struct
  ...
  let unit_intro (store : ExpressionStore.t) : expr_id = Term.unit store

  (** 2. LetUnit: Unit type elimination (sequencing) *)
  let unit_elim (store : ExpressionStore.t) (computation : expr_id)
      (continuation : expr_id) : expr_id =
    Term.let_unit store computation continuation

  (** 3. Tensor: Product type introduction *)
  let product_intro (store : ExpressionStore.t) (left : expr_id)
      (right : expr_id) : expr_id =
    Term.tensor store left right

  (** 4. LetTensor: Product type elimination *)
  let product_elim (store : ExpressionStore.t) (pair : expr_id) (body : expr_id)
      : expr_id =
  ...
  let sum_elim (store : ExpressionStore.t) (scrutinee : expr_id)
      (left_branch : expr_id) (right_branch : expr_id) : expr_id =
    Term.case store scrutinee left_branch right_branch

  (** 8. Lambda: Linear function introduction *)
  let function_intro (store : ExpressionStore.t) (param : string)
      (body : expr_id) : expr_id =
    Term.lambda store param body

  (** 9. Apply: Linear function elimination *)
  let function_elim (store : ExpressionStore.t) (func : expr_id) (arg : expr_id)
      : expr_id =
  ...
  let resource_consume (store : ExpressionStore.t) (resource : expr_id) :
      expr_id =
