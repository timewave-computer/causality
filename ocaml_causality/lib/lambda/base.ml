...
module Value = struct
  ...
  let rec get_type (value : value) : type_inner =
    ...
    | Sum { tag = _; value } -> Sum (get_type value, get_type value) (* Simplified *)
    | Record { fields } -> 
      let field_types = List.map (fun (name, v) -> (name, get_type v)) fields in
  ...
  (** Create record value *)
  let record (fields : (string * value) list) : value = 
    Record { fields }
  ...
  let project_field (value : value) (field_name : string) : value option =
    match value with
    | Record { fields } -> 
      List.assoc_opt field_name fields
    | _ -> None
end
...
module Linearity = struct
  ...
  let check_constraint (_linearity : 'a) (constraint_type : constraint_type) : bool =
...
module LinearResource = struct
  ...
  let consume (resource : 'a t) : 'a =
    if resource.consumed then
      failwith "Linear resource already consumed"
    else
      resource.value
  ...
  let mark_consumed (resource : 'a t) : 'a t =
    { resource with consumed = true }
end 