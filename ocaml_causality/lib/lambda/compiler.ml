(** Layer 1 to Layer 0 Compiler
    ...
    This module compiles content-addressed linear lambda calculus terms to register machine instructions,
    providing the bridge between the functional and imperative layers.
*)
...
let empty_context (store : ExpressionStore.t) : compile_context = {
  next_reg = 1l;  (* Start from register 1, reserve 0 for special use *)
...
let bind_var (ctx : compile_context) (var : string) (reg : int32) : compile_context =
...
let gen_label (ctx : compile_context) (prefix : string) : compile_context * string =
...
let rec compile_expression (ctx : compile_context) (expr_id : expr_id) (target_reg : int32) 
  ...
  | Some expr ->
    ...
    | Core Unit ->
      ...
      Ok (ctx, [])  (* Simplified - would need LoadImmediate instruction *)
    ...
    | Core (Tensor (e1, e2)) ->
      ...
      let tensor_instr = Instruction.Apply {
        fn_reg = Instruction.RegisterId.create 0l;  (* Built-in tensor function *)
    ...
    | Core (LetTensor (pair_expr, body_expr)) ->
      ...
      let destructure_instr = Instruction.Match {
      ...
      compile_expression ctx4 body_expr target_reg >>= fun (ctx5, instrs2) ->
    ...
    | Core (Inl e) ->
      ...
      let inl_instr = Instruction.Apply {
        fn_reg = Instruction.RegisterId.create 0l;  (* Built-in inl function *)
    ...
    | Core (Inr e) ->
      let (ctx1, val_reg) = alloc_register ctx in
      compile_expression ctx1 e val_reg >>= fun (ctx2, instrs) ->
      let inr_instr = Instruction.Apply {
        fn_reg = Instruction.RegisterId.create 0l;  (* Built-in inr function *)
        arg_reg = Instruction.RegisterId.create val_reg;
        out_reg = Instruction.RegisterId.create target_reg;
      } in
    ...
    | Core (Case (scrutinee, left_branch, right_branch)) ->
      ...
      compile_expression ctx6 scrutinee sum_reg >>= fun (ctx7, scrutinee_instrs) ->
      
      let match_instr = Instruction.Match {
      ...
      compile_expression ctx7 left_branch target_reg >>= fun (ctx8, left_instrs) ->
      
      let right_label_instr = Instruction.LabelMarker right_label in
      compile_expression ctx8 right_branch target_reg >>= fun (ctx9, right_instrs) ->
      
      let end_label_instr = Instruction.LabelMarker end_label in
      
      let all_instrs = scrutinee_instrs @ [match_instr; left_label_instr] @ 
                       left_instrs @ [right_label_instr] @ right_instrs @ [end_label_instr] in
      
      Ok (ctx9, all_instrs)
    
    | Core (Lambda (_, _)) ->
      (* Create a function closure - simplified for now *)
      Ok (ctx, [])  (* Would need function table and closure creation *)
    ...
    | Core (Apply (fn_expr, arg_expr)) ->
      ...
      let apply_instr = Instruction.Apply {
    ...
    | Let (var, value_expr, body_expr) ->
      let (ctx1, val_reg) = alloc_register ctx in
      compile_expression ctx1 value_expr val_reg >>= fun (ctx2, val_instrs) ->
      let ctx3 = bind_var ctx2 var val_reg in
      compile_expression ctx3 body_expr target_reg >>= fun (ctx4, body_instrs) ->
      Ok (ctx4, val_instrs @ body_instrs)
    
    | If (cond_expr, then_expr, else_expr) ->
      let (ctx1, cond_reg) = alloc_register ctx in
      
      compile_expression ctx1 cond_expr cond_reg >>= fun (ctx5, cond_instrs) ->
      ...
      compile_expression ctx7 then_expr then_reg >>= fun (ctx8, then_instrs) ->
      compile_expression ctx8 else_expr else_reg >>= fun (ctx9, else_instrs) ->
      
      let select_instr = Instruction.Select {
...
let compile (store : ExpressionStore.t) (expr_id : expr_id) 
  ...
  let target_reg = 0l in  (* Main result register *)
  compile_expression ctx expr_id target_reg >>= fun (_, instrs) ->
  Ok instrs
...
let compile_and_show (store : ExpressionStore.t) (expr_id : expr_id) : string =
  ...
  | Ok instrs -> 
    ...
    (String.concat "\n" (List.mapi (fun i _ -> 
      Printf.sprintf "%d: %s" i "instruction"  (* Would need pretty printer for instructions *)
    ) instrs))
  | Error err -> 
    "Compilation failed: " ^ string_of_error_kind err 