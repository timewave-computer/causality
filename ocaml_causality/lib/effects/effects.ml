(** Layer 2: OCaml Native Effect System Integration
    ...
    This module provides true integration with OCaml 5.0+ native algebraic effects,
    enabling direct-style programming while maintaining linearity and verifiability.
*)
...
let allocate_resource () : bytes =
  Effect.perform (AllocResource ())
...
let run_with_effects (computation : unit -> 'a) : ('a, exn) result =
  try
    Ok (Effect.Deep.match_with computation () causality_handler)
  with
  | exn -> Error exn
