...
let bytes_to_hex (data : bytes) : string =
  ...
  Bytes.to_string data |> String.to_seq |> Seq.map hex_of_char |> List.of_seq
  |> String.concat ""

(** Convert hex string to bytes *)
let hex_to_bytes (hex : string) : bytes = Bytes.of_string hex

(* ------------ VALIDATION ------------ *)

(** Validate that a resource_id is properly formatted *)
let validate_resource_id (rid : resource_id) : bool = Bytes.length rid > 0

(** Validate that an expr_id is properly formatted *)
let validate_expr_id (eid : expr_id) : bool = Bytes.length eid > 0
