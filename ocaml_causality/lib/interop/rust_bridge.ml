...
let intent_to_rust (intent: intent) : string =
  ...
  let flow_to_string (flow : resource_flow) = Printf.sprintf "{\"type\":\"%s\",\"quantity\":%Ld,\"domain\":\"%s\"}" 
    flow.resource_type flow.quantity (Bytes.to_string flow.domain_id) in
  let inputs_str = String.concat "," (List.map flow_to_string intent.inputs) in
  let outputs_str = String.concat "," (List.map flow_to_string intent.outputs) in
  let expr_str = match intent.expression with
    | Some expr_id -> Bytes.to_string expr_id
    | None -> ""
  in
  let hint_str = match intent.hint with
    | Some expr_id -> Bytes.to_string expr_id
    | None -> ""
  in
  Printf.sprintf "{\"id\":\"%s\",\"name\":\"%s\",\"domain_id\":\"%s\",\"priority\":%d,\"inputs\":[%s],\"outputs\":[%s],\"expression\":\"%s\",\"timestamp\":%Ld,\"hint\":\"%s\"}"
...
let rust_to_intent (data: string) : intent =
  ...
  let extract_int_field field_name data =
    ...
    try
      let start_pos = String.index data (String.get pattern 0) in
      let field_start = start_pos + String.length pattern in
      let field_end = try String.index_from data field_start ',' with Not_found -> String.length data - 1 in
      let int_str = String.sub data field_start (field_end - field_start) in
      int_of_string (String.trim int_str)
    with
    | Not_found | Failure _ -> 0
  in
...

  {
    ...
    inputs = []; (* Simplified - would parse array in production *)
    outputs = []; (* Simplified - would parse array in production *)
    expression = (let expr = extract_field "expression" data in 
                  if expr = "" then None else Some (Bytes.of_string expr));
    timestamp = extract_int64_field "timestamp" data;
    hint = (let h = extract_field "hint" data in 
...
let effect_to_rust (effect: effect) : string =
  ...
  let outputs_str = String.concat "," (List.map flow_to_string effect.outputs) in
  let expr_str = match effect.expression with
    | Some expr_id -> Bytes.to_string expr_id
    | None -> ""
  in
  let hint_str = match effect.hint with
    | Some expr_id -> Bytes.to_string expr_id
    | None -> ""
  in
  Printf.sprintf "{\"id\":\"%s\",\"name\":\"%s\",\"domain_id\":\"%s\",\"effect_type\":\"%s\",\"inputs\":[%s],\"outputs\":[%s],\"expression\":\"%s\",\"timestamp\":%Ld,\"hint\":\"%s\"}"
...
let resource_to_rust (resource: resource) : string =
  Printf.sprintf "{\"id\":\"%s\",\"name\":\"%s\",\"domain_id\":\"%s\",\"resource_type\":\"%s\",\"quantity\":%Ld,\"timestamp\":%Ld}"
...
let create_intent_via_rust name domain_id =
  ...
  with
  | exn -> Error (FFIError ("Rust intent creation failed: " ^ Printexc.to_string exn))
...
let process_effect_via_rust effect_name effect_data =
  try
    ...
    Ok (Bytes.to_string rust_data)
  with
  | exn -> Error (FFIError ("Rust effect processing failed: " ^ Printexc.to_string exn))
...
let serialize_lisp_value_for_rust (value: lisp_value) : string =
  let rec serialize = function
    ...
    | ResourceId rid -> Printf.sprintf "{\"type\":\"resource_id\",\"value\":\"%s\"}" (Bytes.to_string rid)
    | ExprId eid -> Printf.sprintf "{\"type\":\"expr_id\",\"value\":\"%s\"}" (Bytes.to_string eid)
    | Bytes b -> Printf.sprintf "{\"type\":\"bytes\",\"value\":\"%s\"}" (Bytes.to_string b)
  in
  serialize value

(** Deserialize lisp_value from Rust *)
let deserialize_lisp_value_from_rust (data: string) : (lisp_value, causality_error) result =
  try
    ...
    if String.contains data '"' then
      ...
      Error (SerializationError "Invalid Rust data format")
  with
  | exn -> Error (SerializationError ("Failed to parse Rust data: " ^ Printexc.to_string exn))

(** Validate Rust bridge connection *)
let validate_rust_bridge () : (bool, causality_error) result =
  try
    let test_intent = create_intent_via_rust "test" (Bytes.of_string "test_domain") in
    match test_intent with
    | Ok _ -> Ok true
    | Error _ -> Ok false
  with
  | exn -> Error (FFIError ("Bridge validation failed: " ^ Printexc.to_string exn))
...
module RustBridgeRegistry = struct
  ...
  let log_operation registry operation success error_msg =
    ...
    if success then
      registry.successful_calls <- registry.successful_calls + 1
    else
      registry.failed_calls <- registry.failed_calls + 1
  ...
  let get_recent_operations registry count =
    let sorted_ops = List.sort (fun a b -> Int64.compare b.timestamp a.timestamp) registry.operations in
