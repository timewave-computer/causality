...
let check_capability capability context =
  let type_matches = match capability.capability_type, context.requested_operation with
  ...
  let resource_matches = 
    ...
    else String.equal pattern context.target_resource
  in
  
  let owner_matches = String.equal capability.owner context.user_id in
  
  let not_expired = match capability.expires_at with
    | None -> true
    | Some expiry -> expiry > 1640995200L (* Current time mock *)
  in
  
  type_matches && domain_matches && resource_matches && owner_matches && not_expired
...
let get_user_capabilities user_id domain_id capabilities =
  List.filter (fun cap -> 
    String.equal cap.owner user_id && 
    Bytes.equal cap.domain_id domain_id
  ) capabilities

(* ------------ CAPABILITY MANAGEMENT ------------ *)

(* Capability creation and revocation functions *)
let create_capability capability_type resource_pattern domain_id owner expires_at =
...
let validate_capability capability =
  let validations = [
    ...
    (String.length capability.resource_pattern > 0, "Resource pattern must not be empty");
...
let capability_to_string capability =
  let type_str = match capability.capability_type with
    | ReadCapability -> "read"
  ...
  in
  Printf.sprintf "Capability(%s, %s, %s, %s)" 
    type_str capability.resource_pattern capability.owner
...
module CapabilityRegistry = struct
  ...
  let revoke_capability registry capability_id =
    registry.capabilities <- List.filter (fun cap -> 
      not (Bytes.equal cap.id capability_id)
    ) registry.capabilities;
    registry.revoked_capabilities <- capability_id :: registry.revoked_capabilities

  let lookup_capability registry capability_id =
    List.find_opt (fun cap -> Bytes.equal cap.id capability_id) registry.capabilities
