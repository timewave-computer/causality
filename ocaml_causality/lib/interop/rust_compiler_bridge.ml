(** OCaml to Rust Compiler Bridge
    ...
    This module provides OCaml functions to call into the Rust
    causality-compiler via C FFI to compile lambda terms to register machine
    instructions. *)
...
let rec encode_term (term : lambda_term) : int * string =
  ...
  | Apply (func, arg) ->
      ...
      ( term_apply
      , Printf.sprintf "%d|%s|%d|%s" func_type func_data arg_type arg_data )
  | Alloc expr ->
...
module TermBuilder = struct
  ...
  let const_lambda var value = Lambda (var, Int value)

  (** Build an allocation of a constant *)
  let alloc_int value = Alloc (Int value)

  (** Build function application with integer argument *)
  let apply_int func_name arg_value = Apply (Symbol func_name, Int arg_value)

  (** Build a simple identity function *)
  let identity () = Lambda ("x", Symbol "x")

  (** Build a constant function *)
  let const_func value = Lambda ("_", Int value)
end
