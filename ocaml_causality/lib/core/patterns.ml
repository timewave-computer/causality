...
(** Resource pattern for matching resources. Corresponds to Rust's
    `ResourcePattern`. *)
...
let matches_pattern (pattern : resource_pattern) (resource_type : str_t)
    (domain : domain_id) : bool =
  let type_matches = String.equal pattern.resource_type resource_type in
  let domain_matches =
    match pattern.domain_id with
...
let filter_by_pattern (pattern : resource_pattern)
    (resources : (str_t * domain_id) list) : (str_t * domain_id) list =
  List.filter
    (fun (resource_type, domain) ->
      matches_pattern pattern resource_type domain)
    resources
...
(** Create a pattern that matches resources of a specific type in a specific
    domain *)
let pattern_for_type_and_domain (resource_type : str_t) (domain : domain_id) :
    resource_pattern =
...
let create_flow (resource_type : str_t) (quantity : int64) (domain : domain_id)
    : resource_flow =
  ...
  List.iter
    (fun flow ->
      ...
      let current_quantity =
        match Hashtbl.find_opt flow_map key with
        | None -> 0L
        | Some existing_flow -> existing_flow.quantity
      in
      let combined_flow =
        { flow with quantity = Int64.add current_quantity flow.quantity }
      in
