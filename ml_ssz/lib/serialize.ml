(**
 * Serialization module for SSZ
 *
 * Provides core functions for encoding and decoding Simple Serialize (SSZ) data
 * with utility functions for reading and writing various integer types.
 *)
 ...
 *)
let write_uint8 buf offset value =
  Bytes.set buf offset (Char.chr value)
...
let write_uint64 buf offset value =
  ...
  write_uint32 buf (offset + 4) (Int64.to_int (Int64.shift_right_logical value 32))

(**
 * Helper to read a uint8 from an offset in a bytes buffer
 *)
let read_uint8 buf offset =
...
let encode_list_with_length typ values =
  ...
  let total_size = List.fold_left (fun acc bytes -> acc + Bytes.length bytes) 0 items_bytes in
