(**
 * Types module for SSZ serialization
 *
 * Defines the core type specifications for Simple Serialize (SSZ) format,
 * including type kinds, size calculations, and serialization interfaces.
 *)
...
(**
 * The kind of serializable type
 * 
 * Used for determining the appropriate serialization strategy
 *)
type kind =
...
(**
 * Type specification for an SSZ type
 * 
 * Provides the metadata and functions needed for serialization
 *)
type 'a t = {
  ...
  decode : bytes -> int -> 'a * int; (** Decoding function, returns value and new offset *)
}
...
(**
 * Sizes and limits from the SSZ specification
 * 
 * Defines constants used throughout the serialization process
 *)
module Constants = struct
  ...
  let default_max_length = 1024 * 1024  (** 1M elements *)
end