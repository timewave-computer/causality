...
let test_fixed_arrays () =
  ...
  List.iteri (fun i value ->
    test_roundtrip (sprintf "id_%d(32_bytes)" i)
      (fun x -> x) (fun x -> x) value  (* IDs are raw bytes *)
  ) TestData.test_ids
...
let test_ocaml_rust_ocaml () =
  ...
  TestData.bool_values |> List.iteri (fun i value ->
    try
      ...
      add_result { test_name = sprintf "ocaml_rust_ocaml_bool_%d" i; success; error = None };
      printf "  %s Bool %b -> %b\n" (if success then "✓" else "✗") value final_result
    with e ->
      add_result { test_name = sprintf "ocaml_rust_ocaml_bool_%d" i; success = false; error = Some (Printexc.to_string e) };
      printf "  ✗ Bool error: %s\n" (Printexc.to_string e)
  );
  
  TestData.u32_values |> take 3 |> List.iteri (fun i value ->
    try
      ...
      let success = (value = final_result) in
      add_result { test_name = sprintf "ocaml_rust_ocaml_u32_%d" i; success; error = None };
      printf "  %s UInt32 %d -> %d\n" (if success then "✓" else "✗") value final_result
    with e ->
      add_result { test_name = sprintf "ocaml_rust_ocaml_u32_%d" i; success = false; error = Some (Printexc.to_string e) };
      printf "  ✗ UInt32 error: %s\n" (Printexc.to_string e)
  );
  
  TestData.string_values |> take 3 |> List.iteri (fun i value ->
    try
      ...
      let success = (value = final_result) in
      add_result { test_name = sprintf "ocaml_rust_ocaml_string_%d" i; success; error = None };
      printf "  %s String '%s' -> '%s'\n" (if success then "✓" else "✗") value final_result
    with e ->
      add_result { test_name = sprintf "ocaml_rust_ocaml_string_%d" i; success = false; error = Some (Printexc.to_string e) };
...
let test_rust_ocaml_rust () =
  ...
  TestData.bool_values |> List.iteri (fun i value ->
    try
      ...
      let success = (value = final_result) in
      add_result { test_name = sprintf "rust_ocaml_rust_bool_%d" i; success; error = None };
      printf "  %s Bool %b -> %b\n" (if success then "✓" else "✗") value final_result
    with e ->
      add_result { test_name = sprintf "rust_ocaml_rust_bool_%d" i; success = false; error = Some (Printexc.to_string e) };
      printf "  ✗ Bool error: %s\n" (Printexc.to_string e)
  );
  ...
  TestData.u32_values |> take 3 |> List.iteri (fun i value ->
    try
      ...
      let success = (value = final_result) in
      add_result { test_name = sprintf "rust_ocaml_rust_u32_%d" i; success; error = None };
      printf "  %s UInt32 %d -> %d\n" (if success then "✓" else "✗") value final_result
    with e ->
      add_result { test_name = sprintf "rust_ocaml_rust_u32_%d" i; success = false; error = Some (Printexc.to_string e) };
      printf "  ✗ UInt32 error: %s\n" (Printexc.to_string e)
  );
  ...
  TestData.string_values |> take 3 |> List.iteri (fun i value ->
    try
      ...
      let success = (value = final_result) in
      add_result { test_name = sprintf "rust_ocaml_rust_string_%d" i; success; error = None };
      printf "  %s String '%s' -> '%s'\n" (if success then "✓" else "✗") value final_result
    with e ->
      add_result { test_name = sprintf "rust_ocaml_rust_string_%d" i; success = false; error = Some (Printexc.to_string e) };
      printf "  ✗ String error: %s\n" (Printexc.to_string e)
  )
...
let print_summary () =
  ...
  if failed > 0 then (
    printf "\nFailed tests:\n";
    !results |> List.filter (fun r -> not r.success) |> List.iter (fun r ->
