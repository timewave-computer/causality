...
let version_to_byte = function V1 -> 1

(* Convert byte to file_version *)
let version_of_byte = function 1 -> Some V1 | _ -> None
...
let header_of_bytes bytes =
  try
    if String.length bytes < 10 then Error InvalidHeader
    else
      let magic = String.sub bytes 0 4 in
      if magic <> tel_graph_magic then Error InvalidHeader
      else
  ...
  try
    ...
    if file_size < 10 then (
      ...
      | Ok header -> (
          if
            (* Verify object type *)
            header.object_type <> expected_type
          then (
            close_in ic;
            ...
            if not (validate_fn data_str) then
              ...
              try Ok (deserialize_fn data_str)
              with exn -> Error (Serialization (Printexc.to_string exn)))
  with
  | Sys_error msg -> Error (Io msg)
  | exn -> Error (Serialization (Printexc.to_string exn))
...
let write_resource_to_file path resource =
  write_object path Resource MockSerialization.serialize_resource
    MockSerialization.is_valid_resource_bytes resource

let read_resource_from_file path =
  read_object path Resource MockSerialization.deserialize_resource
    MockSerialization.is_valid_resource_bytes

let write_resources_to_file path resources =
  write_objects path Resource MockSerialization.serialize_resource
    MockSerialization.is_valid_resource_bytes resources

let read_resources_from_file path =
  read_objects path Resource MockSerialization.deserialize_resource
    MockSerialization.is_valid_resource_bytes

(* Convenience functions for writing Effect objects *)
let write_effect_to_file path effect =
  write_object path Effect MockSerialization.serialize_effect
    MockSerialization.is_valid_effect_bytes effect

let read_effect_from_file path =
  read_object path Effect MockSerialization.deserialize_effect
    MockSerialization.is_valid_effect_bytes

let write_effects_to_file path effects =
  write_objects path Effect MockSerialization.serialize_effect
    MockSerialization.is_valid_effect_bytes effects

let read_effects_from_file path =
  read_objects path Effect MockSerialization.deserialize_effect
    MockSerialization.is_valid_effect_bytes

(* Convenience functions for writing Handler objects *)
let write_handler_to_file path handler =
  write_object path Handler MockSerialization.serialize_handler
    MockSerialization.is_valid_handler_bytes handler

let read_handler_from_file path =
  read_object path Handler MockSerialization.deserialize_handler
    MockSerialization.is_valid_handler_bytes

let write_handlers_to_file path handlers =
  write_objects path Handler MockSerialization.serialize_handler
    MockSerialization.is_valid_handler_bytes handlers

let read_handlers_from_file path =
  read_objects path Handler MockSerialization.deserialize_handler
    MockSerialization.is_valid_handler_bytes

(* Convenience functions for writing Edge objects *)
let write_edge_to_file path edge =
  write_object path Edge MockSerialization.serialize_edge
    MockSerialization.is_valid_edge_bytes edge

let read_edge_from_file path =
  read_object path Edge MockSerialization.deserialize_edge
    MockSerialization.is_valid_edge_bytes

let write_edges_to_file path edges =
  write_objects path Edge MockSerialization.serialize_edge
    MockSerialization.is_valid_edge_bytes edges

let read_edges_from_file path =
  read_objects path Edge MockSerialization.deserialize_edge
    MockSerialization.is_valid_edge_bytes
...
module Testing = struct
  ...
  let test_round_trip object_type obj =
    let serialize_fn, deserialize_fn, validate_fn =
  ...
  let test_resource resource = test_round_trip Resource resource

  (* Test an effect round-trip *)
  let test_effect effect = test_round_trip Effect effect

  (* Test a handler round-trip *)
  let test_handler handler = test_round_trip Handler handler

  (* Test an edge round-trip *)
  let test_edge edge = test_round_trip Edge edge

  (* Print a test result *)
  let print_test_result result =
    let type_str =
      match result.object_type with
      | Resource -> "Resource"
      | Effect -> "Effect"
      | Handler -> "Handler"
      | Edge -> "Edge"
    ...
    let error =
      match result.error_msg with Some msg -> ": " ^ msg | None -> ""
    in
            ...
            try
              for _ = 1 to object_count do
                ...
                let factory =
                  try
                    List.find
                      (fun (t, _) -> object_type_to_byte t = tag)
                      type_factories
