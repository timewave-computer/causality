(** Merkle module for SSZ. Implements merkleization (hash tree root)
    functionality. *)
...
let hash_leaf hash_fn data =
  if Bytes.length data <= Constants.max_chunk_size then
    ...
    let chunks =
      (Bytes.length data + Constants.max_chunk_size - 1)
    ...
    copy_bytes data 0 first_chunk 0
      (min Constants.max_chunk_size (Bytes.length data));
    let first_hash = hash_fn first_chunk in

    if chunks = 1 then first_hash else hash_chunks first_hash 1
...
let merkleize hash_fn chunks =
  ...
  let padded_chunks =
    ...
    let rec extend acc remaining =
      if remaining = 0 then acc else extend (zero_hash :: acc) (remaining - 1)
    in
  ...
  let rec compute_layer_proofs index depth nodes acc =
    ...
    | _ ->
        ...
        let rec hash_pairs acc i = function
          ...
          | [ x ] -> List.rev (hash_nodes hash_fn x zero_hash :: acc)
          | x :: y :: rest ->
              hash_pairs (hash_nodes hash_fn x y :: acc) (i + 1) rest
        in
...
let verify_proof hash_fn root leaf proof index =
  ...
  let rec verify node idx proof_nodes =
    ...
    | sibling :: rest ->
        let combined =
          if idx mod 2 = 0 then hash_nodes hash_fn node sibling
          else hash_nodes hash_fn sibling node
        in
