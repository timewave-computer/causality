(** * Collections module for SSZ serialization * * Implements serialization for
    collection types like arrays, lists, and vectors * in the Simple Serialize
    (SSZ) format used throughout the project. *)
  ...
  ; encode =
      (fun arr ->
        if Array.length arr <> length then
          failwith
            (Printf.sprintf "Expected array of length %d, got %d" length
               (Array.length arr));
  ...
  ; encode =
      (fun lst ->
        ...
        if count > max_length then
          failwith
            (Printf.sprintf "List exceeds maximum length: %d > %d" count
               max_length);
        ...
        | Some element_size ->
            ...
            let result =
              Bytes.create (Constants.bytes_per_length_prefix + data_size)
            in
        ...
        | None ->
            ...
            let encoded_elements =
              List.map (fun elem -> encode element_type elem) lst
            in
            let total_elements_size =
              List.fold_left
                (fun acc bytes -> acc + Bytes.length bytes)
                0 encoded_elements
            in

            (* Calculate total size: prefix + all encoded elements *)
            let total_size =
              Constants.bytes_per_length_prefix + total_elements_size
            in
...
let vector element_type =
  {
    kind = Vector
  ; size = None
  ; (* Size depends on the elements *)
    encode =
      (fun arr ->
        (* Calculate total size after encoding all elements *)
        let encoded_elements =
          Array.map (fun elem -> encode element_type elem) arr
        in
        let total_elements_size =
          Array.fold_left
            (fun acc bytes -> acc + Bytes.length bytes)
  ...
  ; decode =
      (fun _ _ ->
        ...
        failwith
          "Vector decode called directly - use with fixed_array for correct \
           decoding")
  }
  ...
  let pair_type =
    {
      kind = Container
    ; size =
        (match (key_type.size, value_type.size) with
