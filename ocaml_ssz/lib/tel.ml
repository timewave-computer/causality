...
type ssz_id = { bytes : string  (** 32-byte ID *) }
(** SSZ ID representation *)

type ssz_output_definition = { id : ssz_id; type_expr : ssz_id }
...
type ssz_effect = {
  ...
  ; intent_id : ssz_id
  ; effect_type : string
        (** Fixed-size Vector for Str, limit to str_byte_len *)
...
let deserialize_id (bytes : string) : string =
  ...
  if String.length bytes <> id_byte_len then
    failwith
      (Printf.sprintf "Invalid ID length: expected %d, got %d" id_byte_len
         (String.length bytes));
  bytes

(** Serialize/deserialize strings and IDs *)

let serialize_str (s : string) : string = to_fixed_len_string s str_byte_len
...
(** Serialize an ssz_id *)
let serialize_ssz_id (id : ssz_id) : string = id.bytes
...
let create_id (s : string) : ssz_id = { bytes = serialize_id s }

(** Get the string representation of an ID *)
let id_to_string (id : ssz_id) : string = id.bytes

(** Serialize an effect *)
let serialize_effect (effect : ssz_effect) : string =
  let dynamic_expr_bytes =
    match effect.dynamic_expr with
    | Some expr -> serialize_ssz_id expr
    | None -> String.make id_byte_len '\000'
  in

  let scoped_handler_bytes =
    match effect.scoped_handler with
  ...
  let inputs =
    if List.length effect.inputs > max_inputs then
      List.filteri (fun i _ -> i < max_inputs) effect.inputs
    else effect.inputs
  in

  let outputs =
    if List.length effect.outputs > max_outputs then
      List.filteri (fun i _ -> i < max_outputs) effect.outputs
    else effect.outputs
  in

  let constraints =
    if List.length effect.constraints > max_constraints then
      List.filteri (fun i _ -> i < max_constraints) effect.constraints
    else effect.constraints
  in
