(**
 * Container module for SSZ serialization
 *
 * Implements serialization for composite types like structs, records,
 * and other container data structures in the Simple Serialize format.
 *)
 ...
 *)
let field typ getter ~description = {
  typ;
  get = getter;
  description;
}
...
type ('a, 'b) container = {
  ...
  construct: (bytes -> int -> 'a list * int) -> bytes -> int -> 'b * int;  (** Function to construct the value from decoded fields *)
}
...
let fixed_part_size fields =
  List.fold_left (fun acc f -> 
    acc + (match f.typ.size with Some s -> s | None -> Constants.bytes_per_length_offset)
  ) 0 fields
...
let make container =
  ...
  let all_fixed = List.for_all (fun f -> is_fixed_size f.typ) container.fields in
  let size = 
    if all_fixed then
      Some (List.fold_left (fun acc f -> acc + fixed_size f.typ) 0 container.fields)
    else
      None
  in
  
  {
    ...
    encode = (fun value ->
      ...
      if all_fixed then
        let total_size = fixed_size { kind = Container; size; encode = (fun _ -> Bytes.empty); decode = (fun _ _ -> (Obj.magic 0, 0)) } in
        ...
        let _fixed_fields, _variable_fields = List.partition (fun f -> is_fixed_size f.typ) container.fields in
        
        (* Encode all fields *)
        let encoded_fields = List.map (fun field -> 
          let field_value = field.get value in
          encode field.typ field_value
        ) container.fields in
        
        (* Calculate total size of fixed part: fixed fields + offsets *)
        let fixed_size_total = List.fold_left (fun acc f -> 
          acc + (match f.typ.size with Some s -> s | None -> Constants.bytes_per_length_offset)
        ) 0 container.fields in
        
        (* Calculate total size including variable-sized fields *)
        let variable_data_size = 
          List.fold_left2 (fun acc field encoded -> 
            if is_fixed_size field.typ then acc else acc + Bytes.length encoded
    ...
    decode = (fun bytes offset ->
      ...
      container.construct (fun bytes offset ->
        let fixed_offset = ref offset in
        let var_offset_map = ref [] in
        
        (* Process fields to get their values *)
        let field_values = List.map (fun field ->
          if is_fixed_size field.typ then
            (* Decode fixed-size field *)
            let value, new_offset = field.typ.decode bytes !fixed_offset in
        ...
        let field_values = 
          if !var_offset_map = [] then field_values
            ...
            let sorted_var_fields = List.sort (fun (_, o1) (_, o2) -> compare o1 o2) !var_offset_map in
            ...
            let offset_ranges = 
              ...
              | items -> 
                  let rec make_ranges acc = function
                    | [] -> List.rev acc
                    | [(field, offset)] -> List.rev ((field, offset, Bytes.length bytes) :: acc)
                    | (field, offset) :: ((_, next_offset) :: _ as rest) ->
                        make_ranges ((field, offset, next_offset) :: acc) rest
                  in
                  make_ranges [] items
            in
            
            (* Decode each variable-sized field *)
            List.fold_left (fun acc (field, start_offset, _end_offset) ->
              let value, _ = field.typ.decode bytes (offset + start_offset) in
              
              (* Replace the placeholder in field_values with the real value *)
              let field_index = 
                let rec find_index i = function
                  | [] -> failwith "Field not found"
                  | f :: rest -> if f == field then i else find_index (i+1) rest
                in
              ...
              let rec replace i = function
                | [] -> []
                | x :: xs -> if i = field_index then value :: xs else x :: replace (i+1) xs
              in
        ...
        let final_offset =
          if !var_offset_map = [] then end_of_fixed
            ...
            let last_var_field = List.fold_left (fun acc (_, offset) -> max acc offset) 0 !var_offset_map in
            offset + last_var_field  (* This is an approximation - would need real field sizes *)
        in
...
let create1 _type_a ~construct field_a =
  make {
    fields = [field_a];
    construct = (fun decoder bytes offset ->
...
let create4 _type_a _type_b _type_c _type_d ~construct field_a field_b field_c field_d =
  make {
